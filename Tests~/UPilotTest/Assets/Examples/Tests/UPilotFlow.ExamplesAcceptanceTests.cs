...
using CodingRiver.UPilot.Flow.Examples;

namespace CodingRiver.UPilot.Flow
{
    public sealed class UPilotFlowExamplesAcceptanceTests
    {
        [Test]
...
fixture:
  host_window:
    type: CodingRiver.UPilot.Flow.Examples.ExampleBasicLoginWindow
    reopen_if_open: true
steps:
  - action: wait
    duration: '10ms'
";
            ...
            Assert.That(definition.Fixture.HostWindow.Type, Is.EqualTo("CodingRiver.UPilot.Flow.Examples.ExampleBasicLoginWindow"));
        ...
        public IEnumerator HostWindowManager_ReopensWindowAndClearsDirtyState()
        {
            ...
            yield return UPilotFlowTestTaskUtility.Await(openFirstTask, tuple =>
            {
            ...
            yield return UPilotFlowTestTaskUtility.Await(openSecondTask, tuple =>
            {
        ...
        public IEnumerator HostWindowManager_CallsPrepareForAutomatedTest()
        {
            ...
            yield return UPilotFlowTestTaskUtility.Await(openTask, tuple =>
            {
                window = tuple.window;
            });

            Assert.That(window, Is.Not.Null);
            Assert.That(window.titleContent.text, Is.EqualTo("Example Basic Login"));
            window.Close();
            yield return null;
        }
    }
