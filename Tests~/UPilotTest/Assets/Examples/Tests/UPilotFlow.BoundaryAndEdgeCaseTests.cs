...
namespace CodingRiver.UPilot.Flow
{
    ...
    public sealed class UPilotFlowBoundaryTests : UPilotFlowFixture<SampleLoginWindow>
    {
        [UnityTest]
        public IEnumerator PressKeyCombinationAction_RejectsInvalidInput()
        {
            ...
            foreach (string bad in new[] { "A", "Ctrl+Shift", "Ctrl+InvalidKey" })
            {
                ...
                yield return UPilotFlowTestTaskUtility.AwaitFailure(task, ex =>
                {
                    ...
                    Assert.That(((UPilotFlowException)ex).ErrorCode, Is.EqualTo(ErrorCodes.ActionParameterInvalid));
                });
            }
        }
        ...
        public IEnumerator ClickAction_WithMenuItem_FallsBackToDropdownMenu()
        {
            ...
            Task task = action.ExecuteAsync(Root, new ActionContext
            {
                ...
                SimulationSession = new UPilotFlowSimulationSession(),
        ...
        [UnityTest]
        public IEnumerator ScreenshotAction_CapturesFallbackWhenWindowNotFocused()
        {
            yield return null;
            ...
            var context = new ActionContext
            {
                Root = Root,
                CancellationToken = CancellationToken.None,
                ScreenshotManager = new ScreenshotManager(new TestOptions { ScreenshotPath = Path.Combine(Path.GetTempPath(), "UPilotFlowTests") }, () => null),
            };

            Task task = action.ExecuteAsync(Root, context, new Dictionary<string, string> { ["tag"] = "unfocused" });
            yield return UPilotFlowTestTaskUtility.Await(task);
            Assert.That(context.CurrentAttachments, Has.Count.EqualTo(1));
            Assert.That(context.CurrentAttachments[0], Does.Contain("unfocused"));
            Assert.That(File.Exists(context.CurrentAttachments[0]), Is.True);
        }
        ...
        public IEnumerator StepExecutor_LoopExceedsMaxIterations()
        {
            ...
            yield return UPilotFlowTestTaskUtility.Await(loopTask, result =>
            {
...
";
        ...
        public IEnumerator TestRunner_RunSuiteAsync_EmptyDirectoryReturnsZero()
        {
            ...
            yield return UPilotFlowTestTaskUtility.Await(task, suite =>
            {
        ...
        public IEnumerator TestHostWindowManager_ResolveType_MissingAndAmbiguous()
        {
            ...
            Assert.That(tie.InnerException, Is.TypeOf<UPilotFlowException>());
            Assert.That(((UPilotFlowException)tie.InnerException).ErrorCode, Is.EqualTo(ErrorCodes.HostWindowTypeInvalid));
        ...
        public IEnumerator TestHostWindowManager_OpenAsync_DoesNotReopenWhenFlagFalse()
        {
            ...
            yield return UPilotFlowTestTaskUtility.Await(openTask, tuple => first = tuple.window);
            ...
            yield return UPilotFlowTestTaskUtility.Await(secondTask, tuple => second = tuple.window);
        ...
        public IEnumerator WaitForElementAsync_ThrowsWhenRootPanelReleased()
        {
            ...
            {
                Assert.That(ex, Is.TypeOf<UPilotFlowException>());
                Assert.That(((UPilotFlowException)ex).ErrorCode, Is.EqualTo(ErrorCodes.ElementDisposedDuringQuery));
            });
        }
        ...
        public IEnumerator ElementFinder_ThrowsOnNullInputs()
        {
            yield return null;
            var finder = new ElementFinder();
            Assert.Throws<UPilotFlowException>(() => finder.Find(null, Root), $"Expected {ErrorCodes.SelectorEmpty}");
            Assert.Throws<UPilotFlowException>(() => finder.Find(new SelectorCompiler().Compile("#x"), null), $"Expected {ErrorCodes.RootElementMissing}");
        }
        ...
        public IEnumerator RuntimeController_CancelDuringWaitIfPaused()
        {
            ...
            using (var controller = new RuntimeController())
            {
                ...
                cts.CancelAfter(50);
                controller.Stop();
                yield return UPilotFlowTestTaskUtility.Await(waitTask);
        ...
        public IEnumerator FloatingPanelLocator_FindsDropdownMenuItem()
        {
            ...
            yield return UPilotFlowTestTaskUtility.Await(clickAction.ExecuteAsync(Root, new ActionContext
            {
                Root = Root,
                Finder = new ElementFinder(),
                Options = new TestOptions(),
