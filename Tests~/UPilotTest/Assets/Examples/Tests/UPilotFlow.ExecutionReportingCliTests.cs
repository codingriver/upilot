...
{
    public sealed class UPilotFlowExecutionFixtureTests : UPilotFlowFixture<SampleLoginWindow>
    {
        [UnityTest]
...
";
            ...
            yield return UPilotFlowTestTaskUtility.Await(ExecuteYamlStepsAsync(yaml, "continue-on-step-failure.yaml"), result =>
            {
...
";
            ...
            yield return UPilotFlowTestTaskUtility.Await(ExecuteYamlStepsAsync(yaml, "timeout-case.yaml"), result =>
            {
                Assert.That(result.Status, Is.EqualTo(TestStatus.Failed));
...
";
            ...
            yield return UPilotFlowTestTaskUtility.Await(ExecuteYamlStepsAsync(yaml, "fixture-context.yaml"), result =>
            {
    ...
    public sealed class UPilotFlowReportingAndCliTests
    {
        [Test]
        public void MarkdownReporter_WritesCaseSuiteAndArtifactManifest()
        {
            ...
            new UPilotFlowReportService().WriteArtifactManifest(reportRoot);

            Assert.That(File.Exists(Path.Combine(reportRoot, "Cases", "Reporter Case.md")), Is.True);
            Assert.That(File.Exists(Path.Combine(reportRoot, "Cases", "Reporter Case.json")), Is.True);
            Assert.That(File.Exists(Path.Combine(reportRoot, "full_reports.md")), Is.True);
            Assert.That(File.Exists(Path.Combine(reportRoot, "Cases", "suite-editor-suite.json")), Is.True);
            Assert.That(File.Exists(Path.Combine(reportRoot, "Artifacts", "artifacts.json")), Is.True);
            Assert.That(File.ReadAllText(Path.Combine(reportRoot, "Cases", "Reporter Case.md")), Does.Contain("H=OfficialEditorWindowPanelSimulator; P=PanelSimulator; K=PanelSimulator"));
            Assert.That(File.ReadAllText(Path.Combine(reportRoot, "Cases", "Reporter Case.md")), Does.Contain("host=OfficialEditorWindowPanelSimulator; pointer=PanelSimulator; keyboard=PanelSimulator"));
            Assert.That(File.ReadAllText(Path.Combine(reportRoot, "Cases", "Reporter Case.md")), Does.Contain(ScreenshotManager.SourceWindowReadScreenPixel));
        }
...
");
            ...
            {
                "Unity.exe",
                "-upilotFlow.configFile", configPath,
                "-upilotFlow.headed", "false",
                "-upilotFlow.reportPath", "CliReports",
                "-upilotFlow.screenshotOnFailure", "true",
                "-upilotFlow.defaultTimeoutMs", "3000",
                "-upilotFlow.preStepDelayMs", "120",
                "-upilotFlow.requireOfficialHost", "false",
                "-upilotFlow.requireOfficialPointerDriver", "false",
                "-upilotFlow.requireInputSystemKeyboardDriver", "false",
            };
...
");
        ...
        public void CommandLineParser_PrefersCliValuesOverEnvironment()
        {
            ...
            {
                "Unity.exe",
                "-upilotFlow.headed", "true",
                "-upilotFlow.reportPath", "CliReports",
                "-upilotFlow.defaultTimeoutMs", "3300",
            };
        ...
        public void CommandLineParser_ToTestOptions_MapsStrictFlags()
        {
            ...
            try
            {
                ...
                {
                    "Unity.exe",
                    "-upilotFlow.requireOfficialHost", "true",
                    "-upilotFlow.requireOfficialPointerDriver", "true",
                    "-upilotFlow.requireInputSystemKeyboardDriver", "true",
                    "-upilotFlow.preStepDelayMs", "240",
                });
        ...
        public void CommandLineParser_RejectsExplicitYamlPathAndDirectoryTogether()
        {
            ...
            {
                "Unity.exe",
                "-upilotFlow.yamlPath", "Assets/Examples/Yaml/01-basic-login.yaml",
                "-upilotFlow.yamlDirectory", "Assets/Examples/Yaml",
            };
        ...
        public void ProjectSettings_AlwaysEnableVerboseLog_OverridesRuntimeFlags()
        {
            ...
            try
            {
                ...
                TestOptions resolved = UPilotFlowProjectSettingsUtility.ApplyOverrides(new TestOptions
                {
                    Headed = true,
        ...
        {
            Assert.That(YamlTestCaseFilter.Match("sample-01-*", "Assets/Examples/Yaml/sample-01-basic-login.yaml", "Basic Login"), Is.True);
        ...
        {
            HashSet<string> assemblies = UPilotFlowConfigResolver.GetCustomActionAssemblyWhitelist();
            var registry = new ActionRegistry();

            Assert.That(assemblies, Does.Contain("Assembly-CSharp"));
            Assert.That(assemblies, Does.Contain("UPilot.Flow.Tests"));
        ...
        public void ConfigFileLoader_ThrowsOnInvalidYaml()
        {
            ...
            UPilotFlowException ex = Assert.Throws<UPilotFlowException>(() => new ConfigFileLoader().Load(badPath));
        ...
        public void CommandLineParser_ResolvesComplexPriority()
        {
            ...
            string[] args = { "Unity.exe", "-upilotFlow.headed", "true", "-upilotFlow.reportPath", "Cli" };
        ...
        public void ReportPathBuilder_BuildsExpectedPaths()
        {
            ...
            Assert.That(Path.GetFileName(suiteMd), Is.EqualTo("full_reports.md"));
        }
...
");
            ...
            Assert.That(File.Exists(Path.Combine(tempDir, "Artifacts", "artifacts.json")), Is.True);
        }

        private static string CreateTempDirectory()
        {
            string path = Path.Combine(Path.GetTempPath(), "UPilotFlowTests", System.Guid.NewGuid().ToString("N"));
