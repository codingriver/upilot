...
namespace CodingRiver.UPilot.Flow
{
    public sealed class AdvancedControlsWindow : EditorWindow
    {
        ...
        {
            EnsureInspectorData();
            _inspectorData.inspectorTitle = "Draft";
            _inspectorData.inspectorCount = 2;
            _inspectorData.inspectorEnabled = true;
            ...
            var menuBar = new VisualElement { name = "menu-bar" };
            menuBar.AddToClassList("menu-bar");
            var firstMenuItem = new Button { name = "menu-item-1", text = "File" };
            firstMenuItem.AddToClassList("item");
            var secondMenuItem = new Button { name = "menu-item-2", text = "Edit" };
            secondMenuItem.AddToClassList("item");
            menuBar.Add(firstMenuItem);
            menuBar.Add(secondMenuItem);

            var loginButton = new Button { name = "login-button", text = "Login" };
            var contextMenuTarget = new Button { name = "context-menu-target", text = "Context Target" };
            contextMenuTarget.AddManipulator(new ContextualMenuManipulator(evt =>
            {
                evt.menu.AppendAction("Inspect", _ => { });
                evt.menu.AppendAction("Delete", _ => { }, _ => DropdownMenuAction.Status.Disabled);
            }));
            ...
            standaloneScroller.viewDataKey = string.Empty;
            standaloneScroller.slider.viewDataKey = string.Empty;
            ...
            NameSplitViewDragline(splitView);
            ...
            rootVisualElement.Add(contextMenuTarget);
            rootVisualElement.Add(tabView);
            rootVisualElement.Add(tabStatus);
            rootVisualElement.schedule.Execute(() => standaloneScroller.value = 10f).ExecuteLater(0);
            ...
            rootVisualElement.schedule.Execute(() => NameGeneratedEditorChildren(propertyField, inspectorElement, toolbarBreadcrumbs, toolbarPopupSearch)).ExecuteLater(32);
            rootVisualElement.schedule.Execute(() =>
            {
                NameGeneratedEditorChildren(propertyField, inspectorElement, toolbarBreadcrumbs, toolbarPopupSearch);
                NameSplitViewDragline(splitView);
            }).ExecuteLater(128);
        }
        ...
        private static void NameSplitViewDragline(TwoPaneSplitView splitView)
        {
            FieldInfo anchorField = typeof(TwoPaneSplitView).GetField("m_DragLineAnchor", BindingFlags.Instance | BindingFlags.NonPublic);
            if (anchorField?.GetValue(splitView) is VisualElement anchor)
            {
                anchor.name = "workspace-split-dragline";
                if (!anchor.ClassListContains("unity-two-pane-split-view__dragline-anchor"))
        ...
        private VisualElement CreateMultiColumnTreeView()
        {
            try
            {
                ...
                SetPropertyIfWritable(view, "fixedItemHeight", 20f);
                MethodInfo setRootItems = viewType.GetMethods(PublicInstance)
                    .FirstOrDefault(method => method.Name == "SetRootItems" && method.IsGenericMethodDefinition && method.GetParameters().Length == 1);
                setRootItems?.MakeGenericMethod(typeof(string)).Invoke(view, new object[] { _treeItems });
            ...
            {
                Debug.LogWarning($"[UPilot Flow Sample] MultiColumnTreeView creation failed: {ex}");
                return null;
            }
        }
