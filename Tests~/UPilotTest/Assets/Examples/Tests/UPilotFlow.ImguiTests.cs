...
namespace CodingRiver.UPilot.Flow
{
    ...
    public sealed class UPilotFlowImguiTests
    {
        ...
        {
            Assert.Throws<UPilotFlowException>(() => ImguiSelectorCompiler.Compile(""));
            Assert.Throws<UPilotFlowException>(() => ImguiSelectorCompiler.Compile("not_a_selector"));
        }
        ...
        public System.Collections.IEnumerator SnapshotCapture_RealWindow_HasEntries()
        {
            ...
            var window = EditorWindow.GetWindow<Examples.ImguiExampleWindow>();
            window.Show();
            window.Repaint();

            // Install the bridge before repainting; the Unity 6000 MonoHook fallback
            // can only capture frames that occur after the hook is attached.
