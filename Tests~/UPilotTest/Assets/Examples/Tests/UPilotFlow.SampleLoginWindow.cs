...
namespace CodingRiver.UPilot.Flow
{
    public static class SampleWindowAssetLoader
    {
        public static void RebuildWindow(EditorWindow window, string uxmlPath, string ussPath)
        {
            if (window == null)
            {
                throw new UPilotFlowException(ErrorCodes.RootElementMissing, "Sample window is null.");
            }

            VisualTreeAsset tree = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(uxmlPath);
            if (tree == null)
            {
                throw new UPilotFlowException(ErrorCodes.RootElementMissing, $"Missing UXML asset: {uxmlPath}");
            }

            StyleSheet styleSheet = AssetDatabase.LoadAssetAtPath<StyleSheet>(ussPath);
            if (styleSheet == null)
            {
                throw new UPilotFlowException(ErrorCodes.RootElementMissing, $"Missing USS asset: {ussPath}");
            }
    ...
    /// <summary>
    /// Sample window used by UPilotFlow examples and tests.
    /// </summary>
    public sealed class SampleLoginWindow : EditorWindow
    {
        ...
        public void BuildUi()
        {
            ...
            {
                throw new UPilotFlowException(ErrorCodes.RootElementMissing, "Sample login UXML is missing required elements.");
            }
    ...
    public sealed class SampleInteractionWindow : EditorWindow
    {
        ...
        public void BuildUi()
        {
            ...
            {
                throw new UPilotFlowException(ErrorCodes.RootElementMissing, "Sample interaction UXML is missing required elements.");
            }
    ...
    public sealed class CustomLoginAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            {
                throw new UPilotFlowException(ErrorCodes.ActionTargetTypeInvalid, "custom_login target type is not assignable.");
            }
