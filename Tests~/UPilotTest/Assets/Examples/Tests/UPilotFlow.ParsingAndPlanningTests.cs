...
{
    public sealed class UPilotFlowParsingAndPlanningTests
    {
        [Test]
        public void Parser_BuildsExecutionPlan_FromCsvCase()
        {
            ...
            Assert.That(plan.CaseName, Is.EqualTo("02-data-driven-csv"));
...
";
            ...
            UPilotFlowException ex = Assert.Throws<UPilotFlowException>(() => new YamlTestCaseParser().Parse(yaml, "invalid.yaml"));
...
", Path.GetFullPath("Assets/Examples/Yaml/05-custom-action-and-json.yaml"));
        ...
        {
            string tempDirectory = Path.Combine(Path.GetTempPath(), "UPilotFlowTests", System.Guid.NewGuid().ToString("N"));
        ...
        {
            UPilotFlowException ex = Assert.Throws<UPilotFlowException>(() => TestCaseSchemaValidator.Validate(new TestCaseDefinition { Name = "", Steps = new List<StepDefinition> { new StepDefinition { Action = "wait", Duration = "10ms" } } }));
            Assert.That(ex.ErrorCode, Is.EqualTo(ErrorCodes.TestCaseSchemaInvalid));
        }
        ...
        {
            UPilotFlowException ex = Assert.Throws<UPilotFlowException>(() => TestCaseSchemaValidator.Validate(new TestCaseDefinition { Name = "No Steps" }));
            Assert.That(ex.ErrorCode, Is.EqualTo(ErrorCodes.TestCaseSchemaInvalid));
        }
        ...
        {
            UPilotFlowException ex = Assert.Throws<UPilotFlowException>(() => TestCaseSchemaValidator.Validate(new TestCaseDefinition { Name = "Bad Timeout", Steps = new List<StepDefinition> { new StepDefinition { Action = "wait", Duration = "10ms" } }, TimeoutMs = -1 }));
            Assert.That(ex.ErrorCode, Is.EqualTo(ErrorCodes.TestCaseSchemaInvalid));
        }
        ...
        public void SchemaValidator_RejectsEmptyHostWindowType()
        {
            ...
            UPilotFlowException ex = Assert.Throws<UPilotFlowException>(() => TestCaseSchemaValidator.Validate(definition));
            Assert.That(ex.ErrorCode, Is.EqualTo(ErrorCodes.TestCaseSchemaInvalid));
        }
        ...
        {
            Assert.Throws<UPilotFlowException>(() => DurationParser.ParseToMilliseconds("-1ms", "step"));
            Assert.Throws<UPilotFlowException>(() => DurationParser.ParseToMilliseconds("600001ms", "step"));
            Assert.Throws<UPilotFlowException>(() => DurationParser.ParseToMilliseconds("1.5x", "step"));
            Assert.Throws<UPilotFlowException>(() => DurationParser.ParseToMilliseconds(string.Empty, "step"));
        }
        ...
        public void ExecutionPlanBuilder_RejectsOver5000Steps()
        {
            ...
            UPilotFlowException ex = Assert.Throws<UPilotFlowException>(() => new ExecutionPlanBuilder(new SelectorCompiler(), new ActionRegistry()).Build(definition, new TestOptions()));
            Assert.That(ex.ErrorCode, Is.EqualTo(ErrorCodes.TestCaseSchemaInvalid));
        }
        ...
        {
            string tempDir = Path.Combine(Path.GetTempPath(), "UPilotFlowTests", Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(tempDir);
            string path = Path.Combine(tempDir, "case.yml");
            File.WriteAllText(path, "name: Test\n");

            UPilotFlowException ex = Assert.Throws<UPilotFlowException>(() => new YamlTestCaseParser().ParseFile(path));
        ...
        {
            string path = Path.Combine(Path.GetTempPath(), "UPilotFlowTests", Guid.NewGuid().ToString("N"), "missing.yaml");
            UPilotFlowException ex = Assert.Throws<UPilotFlowException>(() => new YamlTestCaseParser().ParseFile(path));
        ...
        {
            UPilotFlowException ex = Assert.Throws<UPilotFlowException>(() => new YamlTestCaseParser().Parse("name: [ broken yaml", "inline.yaml"));
            Assert.That(ex.ErrorCode, Is.EqualTo(ErrorCodes.YamlParseError));
        }
        ...
        public void TestDataResolver_LoadCsv_ColumnMismatchThrows()
        {
            ...
            UPilotFlowException ex = Assert.Throws<UPilotFlowException>(() => TestDataResolver.ResolveRows(definition));
            Assert.That(ex.ErrorCode, Is.EqualTo(ErrorCodes.TestCaseSchemaInvalid));
        }
        ...
        public void SelectorCompiler_RejectsVariousSyntaxErrors()
        {
            ...
            Assert.Throws<UPilotFlowException>(() => compiler.Compile(""), "Empty");
            Assert.Throws<UPilotFlowException>(() => compiler.Compile("#panel > "), "Trailing child combinator");
            Assert.Throws<UPilotFlowException>(() => compiler.Compile("[attr"), "Unclosed bracket");
            Assert.Throws<UPilotFlowException>(() => compiler.Compile("[attr]"), "Missing value in attribute");
            Assert.Throws<UPilotFlowException>(() => compiler.Compile("# "), "Missing identifier after #");
        }
        ...
        {
            Assert.Throws<UPilotFlowException>(() => YamlObjectReader.AsMap("string", "test"));
            Assert.Throws<UPilotFlowException>(() => YamlObjectReader.AsMap(42, "test"));
        }
        ...
        public void YamlObjectReader_GetBool_ThrowsOnInvalidString()
        {
            ...
            Assert.Throws<UPilotFlowException>(() => YamlObjectReader.GetBool(map, "flag", true, false));
        }
