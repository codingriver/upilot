...
{
    public sealed class UIFlowUtilityAndErrorCodeTests
    {
        [Test]
        public void Utility_SanitizeFileName_ReplacesInvalidCharacters()
        {
            Assert.That(UIFlowUtility.SanitizeFileName("a/b|c"), Is.EqualTo("a_b_c"));
            Assert.That(UIFlowUtility.SanitizeFileName(string.Empty), Is.EqualTo("unnamed"));
            Assert.That(UIFlowUtility.SanitizeFileName(null), Is.EqualTo("unnamed"));
        }
        ...
        public void Utility_DurationMs_ComputesCorrectly()
        {
            ...
            Assert.That(UIFlowUtility.DurationMs(start, start.AddMilliseconds(150)), Is.EqualTo(150));
            Assert.That(UIFlowUtility.DurationMs(start, start.AddMilliseconds(-10)), Is.EqualTo(0));
        }

        [Test]
        public void Utility_NullIfWhiteSpace_ReturnsNullForEmptyOrWhitespace()
        {
            Assert.That(UIFlowUtility.NullIfWhiteSpace("  "), Is.Null);
            Assert.That(UIFlowUtility.NullIfWhiteSpace("hello"), Is.EqualTo("hello"));
        }
        ...
        public void Utility_EnsureRelativeTo_BuildsRelativePath()
        {
            ...
            string relative = UIFlowUtility.EnsureRelativeTo(root, file);
        ...
        public void Utility_AppendDirectorySeparator_DoesNotDoubleAppend()
        {
            ...
            Assert.That(UIFlowUtility.AppendDirectorySeparator(path), Is.EqualTo(path));
            Assert.That(UIFlowUtility.AppendDirectorySeparator("Assets"), Is.EqualTo("Assets" + Path.DirectorySeparatorChar));
        }
