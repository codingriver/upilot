...
{
    public sealed class UIFlowParsingAndPlanningTests
    {
        [Test]
...
";
            ...
            UIFlowException ex = Assert.Throws<UIFlowException>(() => new YamlTestCaseParser().Parse(yaml, "invalid.yaml"));
...
", Path.GetFullPath("Assets/Examples/Yaml/05-custom-action-and-json.yaml"));
        ...
        {
            string tempDirectory = Path.Combine(Path.GetTempPath(), "UIFlowTests", System.Guid.NewGuid().ToString("N"));
        ...
        {
            UIFlowException ex = Assert.Throws<UIFlowException>(() => TestCaseSchemaValidator.Validate(new TestCaseDefinition { Name = "", Steps = new List<StepDefinition> { new StepDefinition { Action = "wait", Duration = "10ms" } } }));
            Assert.That(ex.ErrorCode, Is.EqualTo(ErrorCodes.TestCaseSchemaInvalid));
        }
        ...
        {
            UIFlowException ex = Assert.Throws<UIFlowException>(() => TestCaseSchemaValidator.Validate(new TestCaseDefinition { Name = "No Steps" }));
            Assert.That(ex.ErrorCode, Is.EqualTo(ErrorCodes.TestCaseSchemaInvalid));
        }
        ...
        {
            UIFlowException ex = Assert.Throws<UIFlowException>(() => TestCaseSchemaValidator.Validate(new TestCaseDefinition { Name = "Bad Timeout", Steps = new List<StepDefinition> { new StepDefinition { Action = "wait", Duration = "10ms" } }, TimeoutMs = -1 }));
            Assert.That(ex.ErrorCode, Is.EqualTo(ErrorCodes.TestCaseSchemaInvalid));
        }
        ...
        public void SchemaValidator_RejectsEmptyHostWindowType()
        {
            ...
            UIFlowException ex = Assert.Throws<UIFlowException>(() => TestCaseSchemaValidator.Validate(definition));
            Assert.That(ex.ErrorCode, Is.EqualTo(ErrorCodes.TestCaseSchemaInvalid));
        }
        ...
        {
            Assert.Throws<UIFlowException>(() => DurationParser.ParseToMilliseconds("-1ms", "step"));
            Assert.Throws<UIFlowException>(() => DurationParser.ParseToMilliseconds("600001ms", "step"));
            Assert.Throws<UIFlowException>(() => DurationParser.ParseToMilliseconds("1.5x", "step"));
            Assert.Throws<UIFlowException>(() => DurationParser.ParseToMilliseconds(string.Empty, "step"));
        }
        ...
        public void ExecutionPlanBuilder_RejectsOver5000Steps()
        {
            ...
            UIFlowException ex = Assert.Throws<UIFlowException>(() => new ExecutionPlanBuilder(new SelectorCompiler(), new ActionRegistry()).Build(definition, new TestOptions()));
            Assert.That(ex.ErrorCode, Is.EqualTo(ErrorCodes.TestCaseSchemaInvalid));
        }
        ...
        {
            string tempDir = Path.Combine(Path.GetTempPath(), "UIFlowTests", Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(tempDir);
            string path = Path.Combine(tempDir, "case.yml");
            File.WriteAllText(path, "name: Test\n");

            UIFlowException ex = Assert.Throws<UIFlowException>(() => new YamlTestCaseParser().ParseFile(path));
        ...
        {
            string path = Path.Combine(Path.GetTempPath(), "UIFlowTests", Guid.NewGuid().ToString("N"), "missing.yaml");
            UIFlowException ex = Assert.Throws<UIFlowException>(() => new YamlTestCaseParser().ParseFile(path));
        ...
        {
            UIFlowException ex = Assert.Throws<UIFlowException>(() => new YamlTestCaseParser().Parse("name: [ broken yaml", "inline.yaml"));
            Assert.That(ex.ErrorCode, Is.EqualTo(ErrorCodes.YamlParseError));
        }
        ...
        public void TestDataResolver_LoadCsv_ColumnMismatchThrows()
        {
            ...
            UIFlowException ex = Assert.Throws<UIFlowException>(() => TestDataResolver.ResolveRows(definition));
            Assert.That(ex.ErrorCode, Is.EqualTo(ErrorCodes.TestCaseSchemaInvalid));
        }
        ...
        public void SelectorCompiler_RejectsVariousSyntaxErrors()
        {
            ...
            Assert.Throws<UIFlowException>(() => compiler.Compile(""), "Empty");
            Assert.Throws<UIFlowException>(() => compiler.Compile("#panel > "), "Trailing child combinator");
            Assert.Throws<UIFlowException>(() => compiler.Compile("[attr"), "Unclosed bracket");
            Assert.Throws<UIFlowException>(() => compiler.Compile("[attr]"), "Missing value in attribute");
            Assert.Throws<UIFlowException>(() => compiler.Compile("# "), "Missing identifier after #");
        }
        ...
        {
            Assert.Throws<UIFlowException>(() => YamlObjectReader.AsMap("string", "test"));
            Assert.Throws<UIFlowException>(() => YamlObjectReader.AsMap(42, "test"));
        }
        ...
        public void YamlObjectReader_GetBool_ThrowsOnInvalidString()
        {
            ...
            Assert.Throws<UIFlowException>(() => YamlObjectReader.GetBool(map, "flag", true, false));
        }
