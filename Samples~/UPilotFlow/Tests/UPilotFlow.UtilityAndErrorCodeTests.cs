...
{
    public sealed class UPilotFlowUtilityAndErrorCodeTests
    {
        [Test]
        public void Utility_SanitizeFileName_ReplacesInvalidCharacters()
        {
            Assert.That(UPilotFlowUtility.SanitizeFileName("a/b|c"), Is.EqualTo("a_b_c"));
            Assert.That(UPilotFlowUtility.SanitizeFileName(string.Empty), Is.EqualTo("unnamed"));
            Assert.That(UPilotFlowUtility.SanitizeFileName(null), Is.EqualTo("unnamed"));
        }
        ...
        public void Utility_DurationMs_ComputesCorrectly()
        {
            ...
            Assert.That(UPilotFlowUtility.DurationMs(start, start.AddMilliseconds(150)), Is.EqualTo(150));
            Assert.That(UPilotFlowUtility.DurationMs(start, start.AddMilliseconds(-10)), Is.EqualTo(0));
        }

        [Test]
        public void Utility_NullIfWhiteSpace_ReturnsNullForEmptyOrWhitespace()
        {
            Assert.That(UPilotFlowUtility.NullIfWhiteSpace("  "), Is.Null);
            Assert.That(UPilotFlowUtility.NullIfWhiteSpace("hello"), Is.EqualTo("hello"));
        }
        ...
        public void Utility_EnsureRelativeTo_BuildsRelativePath()
        {
            ...
            string relative = UPilotFlowUtility.EnsureRelativeTo(root, file);
        ...
        public void Utility_AppendDirectorySeparator_DoesNotDoubleAppend()
        {
            ...
            Assert.That(UPilotFlowUtility.AppendDirectorySeparator(path), Is.EqualTo(path));
            Assert.That(UPilotFlowUtility.AppendDirectorySeparator("Assets"), Is.EqualTo("Assets" + Path.DirectorySeparatorChar));
        }
