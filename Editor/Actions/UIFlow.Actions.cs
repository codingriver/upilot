...
namespace codingriver.upilot.UIFlow
{
    ...
    public sealed class ActionContext
    {
        ...
        public UIFlowSimulationSession SimulationSession;
        ...
        public void AddAttachment(string path)
        {
            ...
            {
                Codingriver.Logger.LogWarning($"[UIFlow] {ErrorCodes.AttachmentLimitExceeded}: step {CurrentStepId} already has 10 attachments.");
                return;
            }
        ...
        public void Log(string message)
        {
            ...
            {
                Codingriver.Logger.Log($"[UIFlow][{CurrentCaseName}][{CurrentStepId}] {message}");
            }
        }
    ...
    public sealed class ActionRegistry
    {
        ...
        public void Register(string actionName, Type actionType)
        {
            ...
            {
                throw new UIFlowException(ErrorCodes.ActionNameConflict, "Action name cannot be empty.");
            }

            if (!typeof(IAction).IsAssignableFrom(actionType))
            {
                throw new UIFlowException(ErrorCodes.ActionNameConflict, $"Action {actionName} does not implement IAction.");
            }

            if (_actions.ContainsKey(actionName))
            {
                throw new UIFlowException(ErrorCodes.ActionNameConflict, $"Duplicate action name: {actionName}");
            }
        ...
        public IAction Resolve(string actionName)
        {
            ...
            {
                Codingriver.Logger.LogError($"[UIFlow] 未找到动作 \"{actionName}\"，已注册动作数={_actions.Count}。可用动作：{string.Join(", ", _actions.Keys.Take(20))}");
                throw new UIFlowException(ErrorCodes.ActionNotFound, $"Action not found: {actionName}");
            }
            ...
            {
                Codingriver.Logger.LogError($"[UIFlow] 构造动作 \"{actionName}\" 失败: {ex.Message}");
                throw new UIFlowException(ErrorCodes.ActionExecutionFailed, $"Failed to construct action {actionName}: {ex.Message}", ex);
            }
        }
        ...
        {
            HashSet<string> allowedAssemblies = UIFlowConfigResolver.GetCustomActionAssemblyWhitelist();
    ...
    public static class ActionHelpers
    {
        public static string Require(Dictionary<string, string> parameters, string actionName, string key)
        {
            ...
            {
                throw new UIFlowException(ErrorCodes.ActionParameterMissing, $"Action {actionName} is missing parameter {key}.");
            }
        ...
        public static MouseButton ParseMouseButton(Dictionary<string, string> parameters, string actionName, string key = "button", MouseButton defaultValue = MouseButton.LeftMouse)
        {
            ...
            switch (literal.Trim().ToLowerInvariant())
            {
                ...
                default:
                    throw new UIFlowException(ErrorCodes.ActionParameterInvalid, $"Action {actionName} parameter '{key}' is invalid: {literal}");
            }
        }
            ...
            foreach (string part in parts)
            {
                switch (part.Trim().ToLowerInvariant())
                {
                    ...
                    case "meta":
                        ...
                        break;
                    default:
                        throw new UIFlowException(ErrorCodes.ActionParameterInvalid, $"Action {actionName} parameter '{key}' is invalid: {part}");
                }
            }
        ...
        public static string RequireMenuItem(Dictionary<string, string> parameters, string actionName)
        {
            ...
            throw new UIFlowException(ErrorCodes.ActionParameterMissing, $"Action {actionName} is missing parameter item.");
        }
        ...
        public static void DispatchClick(VisualElement element, int clickCount, MouseButton button, EventModifiers modifiers, ActionContext context = null)
        {
            ...
            {
                throw new UIFlowException(ErrorCodes.ActionExecutionFailed, "Click target is unavailable.");
            }
        ...
        public static void DispatchKeyboardEvent(VisualElement target, EventType eventType, KeyCode keyCode, char character = '\0')
        {
            ...
            if (dispatchRoot == null)
            {
                throw new UIFlowException(ErrorCodes.ActionExecutionFailed, "Keyboard event target is unavailable.");
            }
        ...
        public static void DispatchCommandEvent(VisualElement target, EventType eventType, string commandName)
        {
            if (target == null)
            {
                throw new UIFlowException(ErrorCodes.ActionExecutionFailed, "Command event target is unavailable.");
            }

            if (string.IsNullOrWhiteSpace(commandName))
            {
                throw new UIFlowException(ErrorCodes.ActionParameterMissing, "Command name cannot be empty.");
            }
            ...
            switch (eventType)
            {
                ...
                case EventType.ValidateCommand:
                    ...
                    break;
                default:
                    throw new UIFlowException(ErrorCodes.ActionParameterInvalid, $"Unsupported command event type: {eventType}");
            }
        ...
        public static void DispatchMouseEvent(VisualElement target, EventType eventType, Vector2 mousePosition, Vector2 delta, int button = 0, int clickCount = 1, EventModifiers modifiers = EventModifiers.None)
        {
            ...
            if (dispatchRoot == null)
            {
                throw new UIFlowException(ErrorCodes.ActionExecutionFailed, "Mouse event target is unavailable.");
            }
            ...
            switch (eventType)
            {
                ...
                case EventType.MouseMove:
                    ...
                    break;
                default:
                    throw new UIFlowException(ErrorCodes.ActionParameterInvalid, $"Unsupported mouse event type: {eventType}");
            }
        ...
        public static void DispatchWheelEvent(VisualElement target, Vector2 mousePosition, Vector2 delta)
        {
            ...
            if (dispatchRoot == null)
            {
                throw new UIFlowException(ErrorCodes.ActionExecutionFailed, "Wheel event target is unavailable.");
            }
    ...
    public sealed class TypeTextAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            if (element is Label)
            {
                throw new UIFlowException(ErrorCodes.ActionTargetTypeInvalid, $"type_text target type is not writable: {element.GetType().Name}");
            }
    ...
    public sealed class TypeTextFastAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            {
                throw new UIFlowException(ErrorCodes.ActionTargetTypeInvalid, $"type_text_fast target type is not writable: {element.GetType().Name}");
            }
        }
    }
    ...
    public sealed class PressKeyAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            {
                throw new UIFlowException(ErrorCodes.ActionParameterInvalid, "press_key parameter 'key' is invalid.");
            }
    ...
    public sealed class PressKeyCombinationAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            if (parts.Count < 2)
            {
                throw new UIFlowException(ErrorCodes.ActionParameterInvalid, "press_key_combination parameter 'keys' must contain at least two keys separated by '+'.");
            }
            ...
            foreach (string part in parts)
            {
                ...
                {
                    throw new UIFlowException(ErrorCodes.ActionParameterInvalid, $"press_key_combination contains invalid key: {part}");
                }
            }

            if (keyCodes.Count != 1)
            {
                throw new UIFlowException(ErrorCodes.ActionParameterInvalid, "press_key_combination must contain exactly one non-modifier key.");
            }
    ...
    public sealed class ScrollAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            {
                throw new UIFlowException(ErrorCodes.ActionParameterInvalid, "scroll parameter 'delta' is invalid.");
            }
    ...
    public sealed class SelectContextMenuItemAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            if (!selected)
            {
                throw new UIFlowException(ErrorCodes.ActionExecutionFailed, $"Context menu item was not available: {itemName}");
            }
    ...
    public sealed class OpenPopupMenuAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            if (!opened)
            {
                throw new UIFlowException(ErrorCodes.OfficialUiTestFrameworkUnavailable, "open_popup_menu requires official PopupMenuSimulator support.");
            }
    ...
    public sealed class SelectPopupMenuItemAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            if (!selected)
            {
                throw new UIFlowException(ErrorCodes.ActionExecutionFailed, $"Popup menu item was not available: {itemName}");
            }
    ...
    public sealed class AssertMenuItemAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            if (!passed)
            {
                throw new UIFlowException(ErrorCodes.ActionExecutionFailed, $"Menu item was not available or not enabled: {itemName}");
            }
    ...
    public sealed class AssertMenuItemDisabledAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            if (!passed)
            {
                throw new UIFlowException(ErrorCodes.ActionExecutionFailed, $"Menu item was not available or not disabled: {itemName}");
            }
    ...
    public sealed class MenuItemAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            switch (mode.Trim().ToLowerInvariant())
            {
                ...
                case "click":
                    ...
                    {
                        throw new UIFlowException(ErrorCodes.ActionExecutionFailed, $"Menu item was not available: {itemName}");
                    }
                    break;
                case "assert_enabled":
                case "enabled":
                ...
                case "disabled":
                    ...
                    break;
                default:
                    throw new UIFlowException(ErrorCodes.ActionParameterInvalid, $"Action menu_item parameter 'mode' is invalid: {mode}");
            }
        ...
        private static void OpenMenuOrThrow(VisualElement element, string menuKind, ActionContext context, out bool usedFallback)
        {
            ...
            throw new UIFlowException(ErrorCodes.OfficialUiTestFrameworkUnavailable, $"menu_item could not open a {menuKind} menu.");
        }
    ...
    public sealed class FocusAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            {
                throw new UIFlowException(ErrorCodes.ActionExecutionFailed, $"focus failed: {ActionContext.ElementInfo(element)} is not focusable.");
            }
            element.Focus();
    ...
    public sealed class SetValueAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            if (!ActionHelpers.TryAssignFieldValue(element, value))
            {
                throw new UIFlowException(ErrorCodes.ActionTargetTypeInvalid, $"set_value target type is not writable: {element.GetType().Name}");
            }
        ...
        public Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            async Task AssertAsync(string currentSelector, int currentTimeout)
            {
                ...
                while (true)
                {
                    ...
                    if (UIFlowUtility.DurationMs(startedAt, DateTimeOffset.UtcNow) >= currentTimeout)
                    {
                        throw new UIFlowException(ErrorCodes.ElementNotVisible, $"Element is still visible: {currentSelector}");
                    }
    ...
    public sealed class AssertTextAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            if (!string.Equals(actual, expected, StringComparison.Ordinal))
            {
                throw new UIFlowException(ErrorCodes.ActionExecutionFailed, $"assert_text failed: expected '{expected}', actual '{actual}'");
            }
    ...
    public sealed class AssertTextContainsAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            {
                throw new UIFlowException(ErrorCodes.ActionExecutionFailed, $"assert_text_contains failed: missing '{expected}'");
            }
    ...
    public sealed class AssertValueAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
                ...
                && AdvancedActionHelpers.TryConvertStringValue(expected, valueType, out object expectedValue))
            {
                ...
                {
                    throw new UIFlowException(ErrorCodes.ActionExecutionFailed, $"assert_value failed: expected '{expected}', actual '{actualText}'");
                }
            ...
            if (!string.Equals(actual, expected, StringComparison.Ordinal))
            {
                throw new UIFlowException(ErrorCodes.ActionExecutionFailed, $"assert_value failed: expected '{expected}', actual '{actual}'");
            }
        }
    }
    ...
    public sealed class AssertEnabledAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            {
                throw new UIFlowException(ErrorCodes.ActionExecutionFailed, $"assert_enabled failed: {ActionContext.ElementInfo(element)} is disabled");
            }
        }
    }
    ...
    public sealed class AssertDisabledAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            if (element.enabledInHierarchy)
            {
                throw new UIFlowException(ErrorCodes.ActionExecutionFailed, $"assert_disabled failed: {ActionContext.ElementInfo(element)} is enabled");
            }
        }
    }
    ...
    public sealed class AssertPropertyAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            if (property == null)
            {
                throw new UIFlowException(ErrorCodes.ActionParameterInvalid, $"assert_property property is invalid: {propertyName}");
            }
            ...
            {
                throw new UIFlowException(ErrorCodes.ActionExecutionFailed, $"assert_property failed: expected '{expected}', actual '{actual}'");
            }
    ...
    public sealed class ScreenshotAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            {
                throw new UIFlowException(ErrorCodes.ScreenshotSaveFailed, "Screenshot manager is not initialized.");
            }
