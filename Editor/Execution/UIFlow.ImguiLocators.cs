...
namespace codingriver.upilot.UIFlow
{
    ...
    public static class ImguiSnapshotCapture
    {
        ...
        public static ImguiSnapshot Capture(EditorWindow window)
        {
            ...
            {
                Codingriver.Logger.LogWarning($"[UIFlow] IMGUI snapshot capture failed: {ex.Message}");
            }
        ...
        private static bool InitializeReflection()
        {
            try
            {
                ...
                {
                    Codingriver.Logger.LogWarning("[UIFlow] GUILayoutUtility.current field not found via reflection. IMGUI automation unavailable.");
                    return false;
                }
                ...
                {
                    Codingriver.Logger.LogWarning("[UIFlow] GUILayoutGroup or GUILayoutEntry type not found. IMGUI automation unavailable.");
                    return false;
                }
            ...
            {
                Codingriver.Logger.LogWarning($"[UIFlow] IMGUI reflection initialization failed: {ex.Message}");
                return false;
            }
        }
    }
    ...
    public sealed class ImguiExecutionBridge
    {
        ...
        public void CaptureSnapshotEarly()
        {
            ...
            Codingriver.Logger.Log($"[UIFlow] CaptureSnapshotEarly: event={evtType}, entries={count}");
        }
        ...
        private void ScheduleDeferredExecution()
        {
            ...
            _deferredCallback = () =>
            {
                ...
                try
                {
                    Codingriver.Logger.Log($"[UIFlow] ScheduleDeferredExecution: _window={_window?.GetType().Name}@{_window?.GetHashCode()}, _pendingCommands={_pendingCommands.Count}");
                ...
                while (_pendingCommands.Count > 0)
                {
                    ...
                    try
                    {
                        Codingriver.Logger.Log($"[UIFlow] Executing {cmd.GetType().Name}: window={_window?.GetType().Name}@{_window?.GetHashCode()}, snapshot={_lastSnapshot?.Entries?.Count}");
                        cmd.Execute(_window, _lastSnapshot);
                    }
                    catch (Exception ex)
                    {
                        Codingriver.Logger.LogWarning($"[UIFlow] IMGUI command failed: {ex}");
                    }
        ...
        private void ExecutePostCommands()
        {
            while (_pendingCommands.Count > 0)
            {
                ...
                {
                    Codingriver.Logger.LogWarning($"[UIFlow] IMGUI command failed: {ex.Message}");
                }
            }
        }
    }
