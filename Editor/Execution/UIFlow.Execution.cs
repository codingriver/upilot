...
namespace codingriver.upilot.UIFlow
{
    ...
    public static class TestHostWindowManager
    {
        public static async Task<(EditorWindow window, VisualElement root)> OpenAsync(HostWindowDefinition hostWindow)
        {
            ...
            {
                throw new UIFlowException(ErrorCodes.HostWindowTypeInvalid, "Host window type is required.");
            }
            ...
            {
                throw new UIFlowException(ErrorCodes.HostWindowOpenFailed, $"Failed to open host window {hostWindow.Type}: {ex.Message}", ex);
            }

            await EditorAsyncUtility.NextFrameAsync(CancellationToken.None);
            if (window is IUIFlowTestHostWindow preparedWindow)
            {
            ...
            {
                window.Close();
                throw new UIFlowException(ErrorCodes.HostWindowOpenFailed, $"Host window root is missing: {hostWindow.Type}");
            }
        ...
        private static Type ResolveType(string typeName)
        {
            ...
            if (matches.Count > 1)
            {
                ...
                throw new UIFlowException(ErrorCodes.HostWindowTypeInvalid, $"Host window type is ambiguous: {typeName}");
            }

            throw new UIFlowException(ErrorCodes.HostWindowTypeInvalid, $"Host window type was not found: {typeName}");
        }
        ...
        public async Task<StepResult> ExecuteStepAsync(ExecutableStep step, ExecutionContext context, int stepIndex)
        {
            ...
            try
            {
                ...
                if (step.Condition != null)
                {
                    ...
                    {
                        if (verboseLog)
                            Codingriver.Logger.Log($"[UIFlow][{context.CaseName}] 步骤[{stepIndex}] \"{step.DisplayName}\" 条件不满足，跳过");
                        result.Status = TestStatus.Skipped;
                        result.EndedAtUtc = DateTimeOffset.UtcNow.ToString("O");
                        result.DurationMs = UIFlowUtility.DurationMs(startedAt, DateTimeOffset.UtcNow);
                ...
                if (verboseLog)
                    Codingriver.Logger.Log($"[UIFlow][{context.CaseName}] 步骤[{stepIndex}] 开始 \"{step.DisplayName}\" 动作={step.ActionName}{selectorInfo} 超时={step.TimeoutMs}ms");
                ...
                if (context.Options?.PreStepDelayMs > 0)
                {
                    if (verboseLog)
                    {
                        Codingriver.Logger.Log($"[UIFlow][{context.CaseName}] 步骤[{stepIndex}] 调试延迟 {context.Options.PreStepDelayMs}ms");
                    }
                ...
                try
                {
                    ...
                    {
                        if (verboseLog)
                            Codingriver.Logger.Log($"[UIFlow][{context.CaseName}] 步骤[{stepIndex}] 进入循环，最大迭代 {step.Loop.MaxIterations}");
                ...
                {
                    throw new UIFlowException(ErrorCodes.StepTimeout, $"步骤 {step.DisplayName} 执行超时：{step.TimeoutMs}ms");
                }
                finally
                {
            ...
            if (verboseLog)
            {
                ...
                Codingriver.Logger.Log($"[UIFlow][{context.CaseName}] 步骤[{stepIndex}] {statusText} \"{step.DisplayName}\" {result.DurationMs}ms{errorDetail}{screenshotDetail}{driverDetail}");
            }
            ...
            while (LoopConditionMet())
            {
                ...
                if (iterations >= step.Loop.MaxIterations)
                {
                    if (context?.Options?.EnableVerboseLog == true)
                        Codingriver.Logger.Log($"[UIFlow][{context.CaseName}] Loop {step.DisplayName} reached max_iterations {step.Loop.MaxIterations}, exiting gracefully.");
                    break;
                }
                ...
                foreach (ExecutableStep loopStep in step.Loop.Steps)
                {
                    ...
                    {
                        throw new UIFlowException(nested.ErrorCode ?? ErrorCodes.ActionExecutionFailed, nested.ErrorMessage ?? $"循环步骤 {loopStep.DisplayName} 执行失败");
                    }
                }
            }
        }
    ...
    public sealed class TestRunner
    {
        ...
        public async Task<TestResult> RunFileAsync(string yamlPath, TestOptions options = null, VisualElement rootOverride = null, Action<ExecutionContext> onContextReady = null)
        {
            ...
            {
                throw new UIFlowException(ErrorCodes.TestCasePathInvalid, "测试用例路径非法");
            }
            ...
            {
                Codingriver.Logger.LogWarning($"[UIFlow] 追加统一报告失败: {unifiedEx.Message}");
            }
        ...
        public async Task<TestSuiteResult> RunSuiteAsync(string directory, TestOptions options = null, Func<string, string, bool> filter = null, VisualElement rootOverride = null)
        {
            ...
            {
                throw new UIFlowException(ErrorCodes.TestSuiteDirectoryNotFound, $"测试目录不存在：{directory}");
            }
            ...
            if (options.EnableVerboseLog)
                Codingriver.Logger.Log($"[UIFlow] 开始执行测试套件 目录={directory} 文件数={yamlFiles.Length}");
            ...
            foreach (string yamlFile in yamlFiles)
            {
                fileIndex++;
                if (options.EnableVerboseLog)
                    Codingriver.Logger.Log($"[UIFlow] 进度 [{fileIndex}/{yamlFiles.Length}] {yamlFile}");
                ...
                catch (Exception ex)
                {
                    ...
                    testResult = new TestResult
                    {
                        ...
                        ErrorCode = ex is UIFlowException flowException ? flowException.ErrorCode : ErrorCodes.CliExecutionError,
            ...
            if (options.EnableVerboseLog)
                Codingriver.Logger.Log($"[UIFlow] 套件完成 通过={suiteResult.Passed} 失败={suiteResult.Failed} 错误={suiteResult.Errors} 跳过={suiteResult.Skipped} 总计={suiteResult.Total}");
            ...
            try
            {
                ...
                if (options.EnableVerboseLog)
                    Codingriver.Logger.Log($"[UIFlow] 套件报告已生成 {options.ReportOutputPath}");
            }
            catch (Exception reportException)
            {
                Codingriver.Logger.LogError($"[UIFlow] {ErrorCodes.ReportWriteFailed}: {reportException.Message} 路径={options.ReportOutputPath}");
            }
            ...
            {
                Codingriver.Logger.LogWarning($"[UIFlow] 写入统一套件报告失败: {unifiedEx.Message}");
            }
        ...
        {
            options = UIFlowProjectSettingsUtility.ApplyOverrides(options);
            options.Validate();
            ...
            {
                Codingriver.Logger.LogWarning($"[UIFlow] RetryCount={options.RetryCount.Value} 在 V1 中不支持，已忽略。如需重试请在调用方实现。");
            }

            if (options.EnableVerboseLog)
                Codingriver.Logger.Log($"[UIFlow] 解析用例 \"{definition.Name}\" YAML={definition.SourceFile ?? "inline"}");
            ...
            if (root == null)
            {
                throw new UIFlowException(ErrorCodes.RootElementMissing, "未找到可执行的根节点");
            }
            ...
            var simulationSession = new UIFlowSimulationSession();
            if (managedWindow != null)
            {
            ...
            if (options.EnableVerboseLog)
            {
                ...
                Codingriver.Logger.Log($"[UIFlow] 绑定驱动 {driverSummary}");
            }
            ...
            if (options.EnableVerboseLog)
            {
                Codingriver.Logger.Log($"[UIFlow] 开始执行用例 \"{definition.Name}\"");
            }
            ...
            try
            {
                ...
                if (options.EnableVerboseLog)
                    Codingriver.Logger.Log($"[UIFlow] 构建执行计划 步骤数={plan.Steps.Count}");
                ...
                for (int index = 0; index < plan.Steps.Count; index++)
                {
                    ...
                    {
                        stepResult.ScreenshotPath = UIFlowUtility.EnsureRelativeTo(options.ReportOutputPath, stepResult.ScreenshotPath);
                    }
            ...
            finally
            {
                ...
                try
                {
                    ...
                    if (options.EnableVerboseLog)
                        Codingriver.Logger.Log($"[UIFlow] 用例报告已生成 {options.ReportOutputPath}");
                }
                catch (Exception reportException)
                {
                    Codingriver.Logger.LogError($"[UIFlow] {ErrorCodes.ReportWriteFailed}: {reportException.Message} 路径={options.ReportOutputPath}");
                ...
                
                {
                    ...
                    Codingriver.Logger.LogWarning($"[UIFlow] {progressPrefix}用例 \"{definition.Name}\" 完成 状态={result.Status} 耗时={result.DurationMs}ms | {stepSummary}");
                }
            }
