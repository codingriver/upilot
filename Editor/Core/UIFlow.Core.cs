...
{
    /// <summary>
    /// Well-known error codes used by UIFlow.
    /// </summary>
    ...
    /// <summary>
    /// Exception raised by UIFlow operations.
    /// </summary>
    public sealed class UIFlowException : Exception
    {
        public UIFlowException(string errorCode, string message)
            : base(message)
        {
            ErrorCode = errorCode;
        }

        public UIFlowException(string errorCode, string message, Exception innerException)
    ...
    public sealed class TestOptions
    {
        ...
        public void Validate()
        {
            ...
            {
                throw new UIFlowException(ErrorCodes.TestOptionsInvalid, "DefaultTimeoutMs 超出允许范围");
            }

            if (string.IsNullOrWhiteSpace(ReportOutputPath) || string.IsNullOrWhiteSpace(ScreenshotPath))
            {
                throw new UIFlowException(ErrorCodes.TestOptionsInvalid, "输出目录不能为空");
            }

            if (RetryCount.HasValue && (RetryCount.Value < 0 || RetryCount.Value > 5))
            {
                throw new UIFlowException(ErrorCodes.TestOptionsInvalid, "RetryCount 超出允许范围");
            }

            if (PreStepDelayMs < 0 || PreStepDelayMs > 60000)
            {
                throw new UIFlowException(ErrorCodes.TestOptionsInvalid, "PreStepDelayMs 超出允许范围");
            }
        }
    }
    ...
    /// </summary>
    public interface IUIFlowTestHostWindow
    {
