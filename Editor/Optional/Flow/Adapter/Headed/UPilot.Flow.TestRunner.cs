...
namespace CodingRiver.UPilot.Flow
{
    ...
    public sealed class TestRunnerWindow : EditorWindow
    {
        ...
        public static void Open()
        {
            ...
            window.titleContent = new GUIContent("UPilot Flow");
        ...
        private void RunAll()
        {
            _ = RunGuardedAsync(() => RunFilteredAsync(c => true));
        }

        private void RunSelected()
        {
            _ = RunGuardedAsync(RunSelectedAsync);
        }
        ...
        {
            _ = RunGuardedAsync(RunStepAsync);
        }

        private async System.Threading.Tasks.Task RunStepAsync()
        {
            if (_state.IsRunning)
            {
        ...
        {
            _ = RunGuardedAsync(RunFailedAsync);
        }

        private async System.Threading.Tasks.Task RunFailedAsync()
        {
        ...
        private static async System.Threading.Tasks.Task RunGuardedAsync(Func<System.Threading.Tasks.Task> action)
        {
            try
            {
                await action();
        ...
        private async System.Threading.Tasks.Task ExecuteBatchAsync(List<string> yamlPaths, Func<string, bool> filter, HeadedRunMode? runModeOverride = null)
        {
            ...
            try
            {
                ...
                UPilotFlowExecutionBatchResult execution = await new UPilotFlowExecutionService().RunAsync(
                    new UPilotFlowExecutionRequest
                    {
                        ...
                        CaseStarted = (caseIndex, totalCases, yamlPath) =>
                        {
                            ...
                            _state.CurrentCaseName = null;
                            TestRunnerCaseItem caseItem = _state.Cases.FirstOrDefault(c =>
                        ...
                        ContextReady = (yamlPath, context) =>
                        {
                            ...
                            if (caseItem != null)
                            {
                                caseItem.CaseName = context.CaseName;
                            }
                            RefreshUi();
                        },
                        CaseCompleted = (caseIndex, totalCases, yamlPath, result) =>
                        {
                            string relativeYamlPath = TestRunnerPathUtility.MakeProjectRelative(yamlPath);
                            TestRunnerCaseItem caseItem = _state.Cases.FirstOrDefault(c =>
                                StringComparer.OrdinalIgnoreCase.Equals(c.YamlPath, relativeYamlPath));
                            ...
                            if (caseItem != null)
                            {
                                ...
                                StepResult failedStep = result.StepResults?.FirstOrDefault(
                                    step => step.Status == TestStatus.Failed || step.Status == TestStatus.Error);
                                caseItem.FailedStepName = failedStep?.DisplayName;
                                caseItem.FailedStepError = failedStep?.ErrorMessage;
                                caseItem.ReportMarkdownPath = TestRunnerPathUtility.MakeProjectRelative(
                                    reportPaths.BuildCaseMarkdownPath(reportRoot, result.CaseName));
                                caseItem.ReportJsonPath = TestRunnerPathUtility.MakeProjectRelative(
                                    reportPaths.BuildCaseJsonPath(reportRoot, result.CaseName));
                ...
                _state.StatusText = execution.Cancelled ? "Aborted"
            ...
            catch (Exception ex)
            {
                ...
                Codingriver.Logger.LogError($"[UPilot Flow] Test Runner batch error: {ex.Message}");
            }
            finally
            {
                _state.IsRunning = false;
    ...
    public static class TestRunnerPathUtility
    {
        public static string MakeProjectRelative(string path)
        {
            ...
            string projectRoot = UPilotFlowUtility.AppendDirectorySeparator(Path.GetFullPath(Directory.GetCurrentDirectory()));
    ...
    {
        private const string Prefix = "CodingRiver.UPilot.Flow.TestRunner.";
        ...
        public static void Load(TestRunnerViewState state)
        {
            ...
            state.EnableVerboseLog = EditorPrefs.GetBool(Prefix + nameof(TestRunnerViewState.EnableVerboseLog), UPilotFlowMenuItems.IsVerboseLogEnabled);
            state.RequireOfficialHost = EditorPrefs.GetBool(Prefix + nameof(TestRunnerViewState.RequireOfficialHost), UPilotFlowProjectSettings.instance.RequireOfficialHostByDefault);
            state.RequireOfficialPointerDriver = EditorPrefs.GetBool(Prefix + nameof(TestRunnerViewState.RequireOfficialPointerDriver), UPilotFlowProjectSettings.instance.RequireOfficialPointerDriverByDefault);
            state.RequireInputSystemKeyboardDriver = EditorPrefs.GetBool(Prefix + nameof(TestRunnerViewState.RequireInputSystemKeyboardDriver), UPilotFlowProjectSettings.instance.RequireInputSystemKeyboardDriverByDefault);
