...
{
    public sealed class UPilotUIToolkitService
    {
        private readonly UPilotBridge _bridge;

        public UPilotUIToolkitService(UPilotBridge bridge)
        {
            _bridge = bridge;
        }
        ...
        private static UIToolkitDumpResultPayload DumpVisualTree(string targetWindow, int maxDepth)
        {
            ...
            var window = UPilotPlayInputService.FindTargetWindow(targetWindow);
        ...
        private static UIToolkitQueryResultPayload QueryElements(UIToolkitQueryPayload payload)
        {
            ...
            var window = UPilotPlayInputService.FindTargetWindow(payload.targetWindow ?? string.Empty);
            if (window == null) return result;
        ...
        private static GenericOkPayload DispatchEvent(UIToolkitEventPayload payload, string eventType)
        {
            ...
            var mods = UPilotPlayInputService.ParseModifiers(payload.modifiers ?? Array.Empty<string>());
