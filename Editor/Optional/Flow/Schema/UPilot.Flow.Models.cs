...
{
    /// <summary>
    /// Well-known error codes used by CodingRiver.UPilot.Flow.
    /// </summary>
    ...
    /// <summary>
    /// Exception raised by UPilot Flow operations.
    /// </summary>
    public sealed class UPilotFlowException : Exception
    {
        public UPilotFlowException(string errorCode, string message)
            : base(message)
        {
            ErrorCode = errorCode;
        }

        public UPilotFlowException(string errorCode, string message, Exception innerException)
    ...
    public sealed class TestOptions
    {
        ...
        public void Validate()
        {
            ...
            {
                throw new UPilotFlowException(ErrorCodes.TestOptionsInvalid, "DefaultTimeoutMs 超出允许范围");
            }

            if (string.IsNullOrWhiteSpace(ReportOutputPath) || string.IsNullOrWhiteSpace(ScreenshotPath))
            {
                throw new UPilotFlowException(ErrorCodes.TestOptionsInvalid, "输出目录不能为空");
            }

            if (RetryCount.HasValue && (RetryCount.Value < 0 || RetryCount.Value > 5))
            {
                throw new UPilotFlowException(ErrorCodes.TestOptionsInvalid, "RetryCount 超出允许范围");
            }
    ...
    /// </summary>
    public interface IUPilotFlowTestHostWindow
    {
