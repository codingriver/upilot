...
namespace CodingRiver.UPilot.Flow
{
    ...
    /// </summary>
    public abstract class UPilotFlowFixture<TWindow>
        where TWindow : EditorWindow
    {
        private UPilotFlowSimulationSession _simulationSession;
        ...
        public virtual IEnumerator SetUp()
        {
            ...
            if (Window == null)
            {
                throw new UPilotFlowException(ErrorCodes.FixtureWindowCreateFailed, $"测试窗口创建失败：{typeof(TWindow).Name}");
            }
            ...
            if (Root == null)
            {
                throw new UPilotFlowException(ErrorCodes.FixtureRootMissing, $"测试窗口根节点缺失：{typeof(TWindow).Name}");
            }

            Finder = new ElementFinder();
            Screenshot = new ScreenshotManager(CurrentOptions, () => Window);
            _simulationSession = new UPilotFlowSimulationSession();
        ...
        protected async Task<TestResult> ExecuteYamlStepsAsync(string yamlContent, string sourcePath = "fixture-inline.yaml")
        {
            ...
            {
                throw new UPilotFlowException(ErrorCodes.FixtureContextNotReady, "测试基座上下文未初始化");
            }

            if (string.IsNullOrWhiteSpace(yamlContent))
            {
                throw new UPilotFlowException(ErrorCodes.FixtureYamlEmpty, "YAML 内容不能为空");
            }
        ...
        protected Task ExecuteActionAsync(IAction action, Dictionary<string, string> parameters)
        {
            if (!IsWindowReady || Root == null)
            {
                throw new UPilotFlowException(ErrorCodes.FixtureContextNotReady, "Fixture context is not ready.");
            }
