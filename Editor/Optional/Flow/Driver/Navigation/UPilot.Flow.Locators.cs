...
namespace CodingRiver.UPilot.Flow
{
    ...
    public sealed class ElementFinder
    {
        ...
        public FindResult Find(SelectorExpression selector, VisualElement root, bool requireVisible = true)
        {
            if (selector == null)
            {
                throw new UPilotFlowException(ErrorCodes.SelectorEmpty, "选择器不能为空");
            }

            if (root == null)
            {
                throw new UPilotFlowException(ErrorCodes.RootElementMissing, $"根节点不存在，无法查找 {selector.Raw}");
            }

            if (EnableVerboseLog)
                Codingriver.Logger.Log($"[UPilot Flow][Locators] 查找元素 选择器={selector.Raw} 范围={RootInfo(root)} 要求可见={requireVisible}");
            ...
            {
                if (EnableVerboseLog)
                    Codingriver.Logger.Log($"[UPilot Flow][Locators] 找到元素 {ActionContext.ElementInfo(element)} 选择器={selector.Raw}");
            ...
            foreach (VisualElement floatingRoot in FloatingPanelLocator.GetFloatingPanelRoots(root))
            {
                ...
                {
                    if (EnableVerboseLog)
                        Codingriver.Logger.Log($"[UPilot Flow][Locators] 找到元素(浮动面板) {ActionContext.ElementInfo(element)} 选择器={selector.Raw}");
                    return new FindResult
                    {
                        Element = element,
                        FoundAtMs = 0,
            ...
            if (EnableVerboseLog)
                Codingriver.Logger.Log($"[UPilot Flow][Locators] 未找到元素 选择器={selector.Raw} 范围={RootInfo(root)}");
            return new FindResult();
        }
        ...
        public async Task<FindResult> WaitForElementAsync(SelectorExpression selector, VisualElement root, WaitOptions options, CancellationToken cancellationToken)
        {
            ...
            {
                throw new UPilotFlowException(ErrorCodes.TestOptionsInvalid, $"等待超时值非法：{options?.TimeoutMs}");
            }
            ...
            while (true)
            {
                ...
                if (root.panel == null)
                {
                    throw new UPilotFlowException(ErrorCodes.ElementDisposedDuringQuery, $"元素树已释放：{selector.Raw}");
                }

                FindResult result = Find(selector, root, options.RequireVisible);
                if (result.Element != null)
                {
                    result.FoundAtMs = UPilotFlowUtility.DurationMs(startedAt, DateTimeOffset.UtcNow);
                    return result;
                }

                int elapsedMs = UPilotFlowUtility.DurationMs(startedAt, DateTimeOffset.UtcNow);
                if (elapsedMs >= options.TimeoutMs)
                {
                    throw new UPilotFlowException(ErrorCodes.ElementWaitTimeout, $"等待元素超时：{selector.Raw}，超时 {options.TimeoutMs}ms");
                }
        ...
        public bool Exists(SelectorExpression selector, VisualElement root, bool requireVisible = true)
        {
            ...
            if (EnableVerboseLog)
                Codingriver.Logger.Log($"[UPilot Flow][Locators] 检查存在 选择器={selector?.Raw} => {exists}");
            return exists;
        }
