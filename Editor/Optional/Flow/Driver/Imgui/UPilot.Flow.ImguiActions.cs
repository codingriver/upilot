...
namespace CodingRiver.UPilot.Flow
{
    ...
    public static class ImguiActionHelper
    {
        public static EditorWindow ResolveWindow(ActionContext context, string actionName)
        {
            ...
            if (window == null)
                throw new UPilotFlowException(ErrorCodes.HostWindowOpenFailed, $"No active EditorWindow for {actionName}.");
            return window;
        }
        ...
        public static ImguiSnapshotEntry RequireEntry(Dictionary<string, string> parameters, ImguiSnapshot snapshot)
        {
            if (!parameters.TryGetValue("selector", out string selectorText))
                throw new UPilotFlowException(ErrorCodes.SelectorInvalid, "IMGUI action requires 'selector' parameter.");

            var selector = ImguiSelectorCompiler.Compile(selectorText);
            var entry = ImguiElementLocator.Find(snapshot, selector);
            if (entry == null)
                throw new UPilotFlowException(ErrorCodes.ElementNotFound,
        ...
        public static void SendMouseEvent(EditorWindow window, Vector2 localPos, EventType eventType, int button = 0)
        {
            ...
            Codingriver.Logger.Log($"[UPilot Flow] SendMouseEvent {eventType} at {localPos} (window={windowPos}, offset={offset}) for {window?.GetType().Name}");
            var evt = new Event
            {
                type = eventType,
                button = button,
    ...
    public sealed class ImguiTypeAction : IAction
    {
        ...
        private async Task ExecuteAsyncInternal(ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            if (!parameters.TryGetValue("text", out string text))
                throw new UPilotFlowException(ErrorCodes.ActionParameterMissing, "imgui_type requires 'text' parameter.");
    ...
    public sealed class ImguiAssertTextAction : IAction
    {
        ...
        private async Task ExecuteAsyncInternal(ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            if (!parameters.TryGetValue("text", out string expectedText))
                throw new UPilotFlowException(ErrorCodes.ActionParameterMissing, "imgui_assert_text requires 'text' parameter.");
            ...
            public override void Execute(EditorWindow window, ImguiSnapshot snapshot)
            {
                try
                {
                    if (!_parameters.TryGetValue("selector", out string selectorText))
                        throw new UPilotFlowException(ErrorCodes.SelectorInvalid, "imgui_assert_text requires 'selector' parameter.");
                    ...
                    if (entry == null)
                    {
                        ...
                        string candidatesStr = candidateTexts.Count > 0
                            ...
                            : "(no candidates)";
                        _tcs.TrySetException(new UPilotFlowException(ErrorCodes.AssertionFailed,
    ...
    public sealed class ImguiWaitAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            if (!parameters.TryGetValue("selector", out string selectorText))
                throw new UPilotFlowException(ErrorCodes.SelectorInvalid, "imgui_wait requires 'selector' parameter.");
            ...
            try
            {
                ...
                // this as a timeout.
                throw new UPilotFlowException(ErrorCodes.StepTimeout,
    ...
    public sealed class ImguiSelectOptionAction : IAction
    {
        ...
        private async Task ExecuteAsyncInternal(ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            if (!parameters.TryGetValue("selector", out string selectorText))
                throw new UPilotFlowException(ErrorCodes.SelectorInvalid, "imgui_select_option requires 'selector' parameter.");
            ...
            if (optionIndex < 0 && string.IsNullOrEmpty(optionText))
                throw new UPilotFlowException(ErrorCodes.ActionParameterMissing, "imgui_select_option requires 'option' or 'index' parameter.");
    ...
    public sealed class ImguiAssertValueAction : IAction
    {
        ...
        private async Task ExecuteAsyncInternal(ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            if (!parameters.TryGetValue("expected", out string expectedValue))
                throw new UPilotFlowException(ErrorCodes.ActionParameterMissing, "imgui_assert_value requires 'expected' parameter.");
    ...
    public sealed class ImguiReadValueAction : IAction
    {
        public async Task ExecuteAsync(VisualElement root, ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            if (!parameters.TryGetValue("selector", out string selectorText))
                throw new UPilotFlowException(ErrorCodes.SelectorInvalid, "imgui_read_value requires 'selector' parameter.");
            ...
            var snapshot = bridge.GetLastSnapshot();
            if (snapshot == null)
                throw new UPilotFlowException(ErrorCodes.ElementNotFound, "IMGUI snapshot not available.");

            var selector = ImguiSelectorCompiler.Compile(selectorText);
            var entry = ImguiElementLocator.Find(snapshot, selector);
            if (entry == null)
                throw new UPilotFlowException(ErrorCodes.ElementNotFound, $"IMGUI element not found: {selectorText}");
    ...
    public sealed class ImguiPressKeyAction : IAction
    {
        ...
        private async Task ExecuteAsyncInternal(ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            {
                throw new UPilotFlowException(ErrorCodes.ActionParameterInvalid, $"imgui_press_key parameter 'key' is invalid: '{key}'");
            }
    ...
    public sealed class ImguiPressKeyCombinationAction : IAction
    {
        ...
        private async Task ExecuteAsyncInternal(ActionContext context, Dictionary<string, string> parameters)
        {
            ...
            if (parts.Count < 2)
            {
                throw new UPilotFlowException(ErrorCodes.ActionParameterInvalid, "imgui_press_key_combination parameter 'keys' must contain at least two keys separated by '+'.");
            }
            ...
            foreach (string part in parts)
            {
                ...
                {
                    throw new UPilotFlowException(ErrorCodes.ActionParameterInvalid, $"Unknown key in combination: '{part}'");
                }
            }

            if (keyCodes.Count == 0)
                throw new UPilotFlowException(ErrorCodes.ActionParameterInvalid, "imgui_press_key_combination must contain at least one non-modifier key.");
