...
namespace CodingRiver.UPilot.Flow
{
    ...
    public static class ImguiBridgeRegistry
    {
        ...
        public static void InstallOnGuiHook(EditorWindow window, ImguiExecutionBridge bridge)
        {
            ...
            if (onguiMethod == null)
            {
                Codingriver.Logger.LogWarning($"[UPilot Flow] Could not find OnGUI method on {windowType.Name}");
                return;
            }
            ...
            try
            {
                ...
                Codingriver.Logger.Log($"[UPilot Flow] MonoHook installed on {windowType.Name}.OnGUI");
            }
            catch (Exception ex)
            {
                Codingriver.Logger.LogWarning($"[UPilot Flow] Failed to install MonoHook on {windowType.Name}.OnGUI: {ex.Message}");
            }
        }
        ...
        public static void UninstallOnGuiHook(EditorWindow window)
        {
            ...
            if (!hasOtherWindowsOfType && Hooks.TryGetValue(windowType, out var hook))
            {
                try
                {
                    ...
                    Codingriver.Logger.Log($"[UPilot Flow] MonoHook uninstalled from {windowType.Name}.OnGUI");
                }
                catch (Exception ex)
                {
                    Codingriver.Logger.LogWarning($"[UPilot Flow] Failed to uninstall MonoHook: {ex.Message}");
                }
            }
        }
        ...
        private static void OnGUIProxy(EditorWindow __this)
        {
            ...
            Codingriver.Logger.LogError("[UPilot Flow] OnGUIProxy should never be called directly!");
        }
        ...
        public static void InvokeOriginalOnGUI(EditorWindow window)
        {
            ...
            try
            {
                Codingriver.Logger.Log($"[UPilot Flow] InvokeOriginalOnGUI calling OnGUIProxy for {window.GetType().Name}, Event.current={Event.current?.type}");
                OnGUIProxy(window);
                Codingriver.Logger.Log($"[UPilot Flow] InvokeOriginalOnGUI OnGUIProxy returned for {window.GetType().Name}");
            }
            catch (Exception ex) when (ex.GetType().Name.Contains("ExitGUIException") || ex.GetType().Name.Contains("TargetInvocationException"))
            {
                Codingriver.Logger.Log($"[UPilot Flow] InvokeOriginalOnGUI caught {ex.GetType().Name} for {window.GetType().Name}");
            }
        }
        ...
        private static void InstallGetRectHook()
        {
            ...
            if (getRectMethod != null)
            {
                try
                {
                    ...
                    Codingriver.Logger.Log("[UPilot Flow] MonoHook installed on GUILayoutUtility.GetRect");
                    return;
                }
                catch (Exception ex)
                {
                    Codingriver.Logger.LogWarning($"[UPilot Flow] Failed to install MonoHook on GUILayoutUtility.GetRect: {ex.Message}");
                }
            }
            ...
            {
                Codingriver.Logger.LogWarning("[UPilot Flow] Could not find GUILayoutUtility.DoGetRect for hooking.");
                return;
            }
            ...
            try
            {
                ...
                _getRectHook.Install();
                Codingriver.Logger.Log("[UPilot Flow] MonoHook installed on GUILayoutUtility.DoGetRect");
            }
            catch (Exception ex)
            {
                Codingriver.Logger.LogWarning($"[UPilot Flow] Failed to install MonoHook on GUILayoutUtility.DoGetRect: {ex.Message}");
            }
        }
        ...
        private static Rect DoGetRectReplacement(GUIContent content, GUIStyle style, GUILayoutOption[] options)
        {
            ...
            if (_currentOnGUIWindow != null)
            {
                ...
                Codingriver.Logger.Log($"[UPilot Flow] DoGetRectReplacement: style={styleName}, content={content?.text}, rect={result}");
        ...
        private static void InstallEditorPopupHook()
        {
            ...
            if (popupMethod == null)
            {
                Codingriver.Logger.LogWarning("[UPilot Flow] Could not find EditorGUILayout.Popup for hooking.");
                return;
            }
            ...
            try
            {
                ...
                Codingriver.Logger.Log("[UPilot Flow] MonoHook installed on EditorGUILayout.Popup");
            }
            catch (Exception ex)
            {
                Codingriver.Logger.LogWarning($"[UPilot Flow] Failed to install MonoHook on EditorGUILayout.Popup: {ex.Message}");
            }
        }
        ...
        private static void InstallGuiStyleDrawHook()
        {
            ...
            if (drawMethod == null)
            {
                Codingriver.Logger.LogWarning("[UPilot Flow] Could not find GUIStyle.Draw for hooking.");
                return;
            }
            ...
            try
            {
                ...
                Codingriver.Logger.Log("[UPilot Flow] MonoHook installed on GUIStyle.Draw");
            }
            catch (Exception ex)
            {
                Codingriver.Logger.LogWarning($"[UPilot Flow] Failed to install MonoHook on GUIStyle.Draw: {ex.Message}");
            }
        }
        ...
        {
            Codingriver.Logger.LogError("[UPilot Flow] DrawProxy should never be called directly!");
        }
