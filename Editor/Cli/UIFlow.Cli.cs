...
namespace codingriver.upilot.UIFlow
{
    ...
    public sealed class ConfigFileLoader
    {
        ...
        public Dictionary<string, object> Load(string path)
        {
            ...
            {
                throw new UIFlowException(ErrorCodes.CliConfigFileInvalid, $"配置文件解析失败：{path}，{ex.Message}", ex);
            }
        }
    }

    public static class UIFlowConfigResolver
    {
        ...
        {
            return Path.Combine(Directory.GetCurrentDirectory(), ".uiflow.json");
        }
    ...
    /// <summary>
    /// Parses UIFlow command-line parameters.
    /// </summary>
    public sealed class CommandLineOptionsParser
    {
        ...
        public CliOptions Parse(string[] args = null, IDictionary<string, string> environmentVariables = null)
        {
            ...
            for (int index = 0; index < args.Length; index++)
            {
                ...
                if (!current.StartsWith("-uiFlow.", StringComparison.OrdinalIgnoreCase))
                {
                    continue;
                }
                ...
                {
                    throw new UIFlowException(ErrorCodes.CliArgumentInvalid, $"命令行参数非法：{current}");
                }

                string key = current.Substring(1);
                if (raw.ContainsKey(key))
                {
                    throw new UIFlowException(ErrorCodes.CliArgumentInvalid, $"命令行参数非法：重复参数 {key}");
                }
            ...
            string configFile = ReadString(
                ...
                "uiFlow.configFile",
                "configFile",
                UIFlowConfigResolver.GetDefaultConfigFilePath());

            Dictionary<string, object> config = _configFileLoader.Load(configFile);
            var options = new CliOptions
            {
                YamlPath = ReadString(raw, environment, config, "uiFlow.yamlPath", "yamlPath", null),
                YamlDirectory = ReadString(raw, environment, config, "uiFlow.yamlDirectory", "yamlDirectory", null),
                Headed = ReadBool(raw, environment, config, "uiFlow.headed", "headed", true),
                ReportPath = ReadString(raw, environment, config, "uiFlow.reportPath", "reportPath", "Reports"),
                ScreenshotOnFailure = ReadBool(raw, environment, config, "uiFlow.screenshotOnFailure", "screenshotOnFailure", true),
                ScreenshotPath = ReadString(raw, environment, config, "uiFlow.screenshotPath", "screenshotPath", null),
                TestFilter = ReadString(raw, environment, config, "uiFlow.testFilter", "testFilter", null),
                StopOnFirstFailure = ReadBool(raw, environment, config, "uiFlow.stopOnFirstFailure", "stopOnFirstFailure", false),
                ContinueOnStepFailure = ReadBool(raw, environment, config, "uiFlow.continueOnStepFailure", "continueOnStepFailure", false),
                DefaultTimeoutMs = ReadInt(raw, environment, config, "uiFlow.defaultTimeoutMs", "defaultTimeoutMs", 3000),
                PreStepDelayMs = ReadInt(raw, environment, config, "uiFlow.preStepDelayMs", "preStepDelayMs", 0),
                RequireOfficialHost = ReadBool(raw, environment, config, "uiFlow.requireOfficialHost", "requireOfficialHost", false),
                RequireOfficialPointerDriver = ReadBool(raw, environment, config, "uiFlow.requireOfficialPointerDriver", "requireOfficialPointerDriver", false),
                RequireInputSystemKeyboardDriver = ReadBool(raw, environment, config, "uiFlow.requireInputSystemKeyboardDriver", "requireInputSystemKeyboardDriver", false),
                EnableVerboseLog = ReadBool(raw, environment, config, "uiFlow.verbose", "verbose", false),
            ...
            {
                throw new UIFlowException(ErrorCodes.CliArgumentInvalid, "-batchmode 模式已被禁用，请使用带窗口的编辑器模式执行测试。");
            }
            ...
            {
                throw new UIFlowException(ErrorCodes.CliFilterInvalid, "测试过滤表达式过长");
            }

            if (!string.IsNullOrWhiteSpace(options.YamlPath) && !string.IsNullOrWhiteSpace(options.YamlDirectory))
            {
                throw new UIFlowException(ErrorCodes.CliArgumentInvalid, "yamlPath 与 yamlDirectory 不能同时指定");
            }

            if (options.DefaultTimeoutMs < 100 || options.DefaultTimeoutMs > 600000)
            {
                throw new UIFlowException(ErrorCodes.CliArgumentInvalid, "defaultTimeoutMs 超出允许范围");
            }
        ...
        private static bool ParseBool(string name, string value)
        {
            ...
            {
                throw new UIFlowException(ErrorCodes.CliArgumentInvalid, $"参数 {name} 的布尔值非法：{value}");
            }
        ...
        private static int ParseInt(string name, string value)
        {
            ...
            {
                throw new UIFlowException(ErrorCodes.CliArgumentInvalid, $"参数 {name} 的数值非法：{value}");
            }
    ...
    public sealed class CiArtifactManifestWriter
    {
        ...
        public void Write(string reportRootPath)
        {
            ...
            {
                throw new UIFlowException(ErrorCodes.CliReportPathInvalid, $"报告目录不可写：{reportRootPath}");
            }
            ...
            foreach (string file in Directory.GetFiles(fullRoot, "*.*", SearchOption.AllDirectories))
            {
                ...
                paths.Add(UIFlowUtility.EnsureRelativeTo(fullRoot, file));
            }
    ...
    /// </summary>
    public static class UIFlowCliEntry
    {
        /// <summary>
        ...
        public static async Task<int> RunAllAsync(string[] args = null)
        {
            ...
            Codingriver.Logger.Log($"[UIFlow] CLI 批量执行开始 配置来源={configSource}");
            try
            {
                ...
                Codingriver.Logger.Log($"[UIFlow] CLI 执行完成 退出码={exitCode}");
            }
            catch (UIFlowException ex)
            {
                exitCode = ex.ErrorCode == ErrorCodes.CliTestsFailed ? 1 : 2;
                Codingriver.Logger.LogError($"[UIFlow] CLI 执行异常 退出码={exitCode} 错误码={ex.ErrorCode}: {ex.Message}");
            }
            catch (Exception ex)
            {
                exitCode = 2;
                Codingriver.Logger.LogError($"[UIFlow] CLI 执行异常 退出码={exitCode}: {ex.Message}");
