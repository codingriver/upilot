// -----------------------------------------------------------------------
// Upilot Editor - UIFlow MCP bridge
...
{
    [Serializable]
    public sealed class UIFlowRunMessage
    {
        public UIFlowRunPayload payload;
    }

    [Serializable]
    public sealed class UIFlowRunPayload
    {
    ...
    [Serializable]
    public sealed class UIFlowResultsMessage
    {
        public UIFlowResultsPayload payload;
    }

    [Serializable]
    public sealed class UIFlowResultsPayload
    {
        public string executionId = "";
    }

    [Serializable]
    public sealed class UIFlowCancelMessage
    {
        public UIFlowCancelPayload payload;
    }

    [Serializable]
    public sealed class UIFlowCancelPayload
    {
        public string executionId = "";
    }

    [Serializable]
    public sealed class UIFlowStepResultPayload
    {
    ...
    [Serializable]
    public sealed class UIFlowCaseResultPayload
    {
        ...
        public List<string> attachments = new List<string>();
        public List<UIFlowStepResultPayload> stepResults = new List<UIFlowStepResultPayload>();
        public UIFlowStepResultPayload failedStep;
    }

    [Serializable]
    public sealed class UIFlowExecutionResultPayload
    {
        public string executionId;
        public string status;
        ...
        public List<UIFlowCaseResultPayload> cases = new List<UIFlowCaseResultPayload>();
    }

    public sealed class UpilotUIFlowService
    {
        private const int MaxYamlPaths = 1000;

        private readonly UpilotBridge _bridge;
        private readonly ConcurrentDictionary<string, UIFlowExecutionResultPayload> _executions = new();
        ...
        private UIFlow.ExecutionContext _activeContext;

        public UpilotUIFlowService(UpilotBridge bridge)
        {
            _bridge = bridge;
            Logger.Log("[UIFlow] UIFlowService 初始化");
            // Register the supplier that feeds a freshly-opened TestRunnerWindow
            // with the current execution snapshot.
            UIFlow.TestRunnerWindow.OnWindowOpened = SyncWindowOnOpen;
        }

        public void RegisterCommands()
        {
            _bridge.Router.Register("uiflow.run", HandleRunAsync);
            _bridge.Router.Register("uiflow.results", HandleResultsAsync);
            _bridge.Router.Register("uiflow.cancel", HandleCancelAsync);
            _bridge.Router.Register("uiflow.force_reset", HandleForceResetAsync);
        }

        private async Task HandleRunAsync(string id, string json, CancellationToken token)
        {
            Logger.Log("UIFlow", $"开始运行测试 id={id}");
            var payload = JsonUtility.FromJson<UIFlowRunMessage>(json)?.payload ?? new UIFlowRunPayload();
            ...
            if (isBusy)
            {
                await _bridge.SendErrorAsync(id, "EDITOR_BUSY", "A UIFlow execution is already running.", token, "uiflow.run");
                return;
            }
            ...
            {
                await _bridge.SendErrorAsync(id, "INVALID_PARAMS", ex.Message, token, "uiflow.run");
                return;
            }
            ...
            var execution = new UIFlowExecutionResultPayload
            {
            ...
            opCtx?.Step("Queued UIFlow execution", $"executionId={executionId} files={yamlPaths.Count}");
            ...
            await _bridge.SendResultAsync(id, "uiflow.run", CloneExecution(execution), token);
        }
        ...
        {
            var payload = JsonUtility.FromJson<UIFlowResultsMessage>(json)?.payload ?? new UIFlowResultsPayload();
            if (string.IsNullOrWhiteSpace(payload.executionId))
            {
                await _bridge.SendErrorAsync(id, "INVALID_PARAMS", "executionId is required.", token, "uiflow.results");
                return;
            }

            if (!_executions.TryGetValue(payload.executionId, out UIFlowExecutionResultPayload execution))
            {
                await _bridge.SendErrorAsync(id, "NOT_FOUND", $"Execution not found: {payload.executionId}", token, "uiflow.results");
                return;
            }
            ...
            if (prevStatus != currStatus)
            {
                ...
                Logger.Log("UIFlow", $"执行状态变化 executionId={payload.executionId[..Math.Min(8, payload.executionId.Length)]} {prevStatus}->{currStatus} 用例={execution.currentCaseName}");
            ...
            await _bridge.SendResultAsync(id, "uiflow.results", CloneExecution(execution), token);
        }

        private async Task HandleForceResetAsync(string id, string json, CancellationToken token)
        {
            Logger.Log("UIFlow", $"强制重置执行状态: id={id}");

            UIFlow.ExecutionContext contextToDispose = null;
            ...
            if (contextToDispose != null)
            {
                EditorApplication.delayCall += () =>
                {
                    ...
                    {
                        Logger.LogWarning("UIFlow", $"Dispose during force reset: {ex.Message}");
                    }
                };
            }
            ...
            var payload = new UIFlowExecutionResultPayload
            {
            ...
            await _bridge.SendResultAsync(id, "uiflow.force_reset", payload, token);
        }

        private async Task HandleCancelAsync(string id, string json, CancellationToken token)
        {
            Logger.Log("UIFlow", $"取消测试执行: id={id}");
            var payload = JsonUtility.FromJson<UIFlowCancelMessage>(json)?.payload ?? new UIFlowCancelPayload();
            if (string.IsNullOrWhiteSpace(payload.executionId))
            {
                await _bridge.SendErrorAsync(id, "INVALID_PARAMS", "executionId is required.", token, "uiflow.cancel");
                return;
            }

            if (!_executions.TryGetValue(payload.executionId, out UIFlowExecutionResultPayload execution))
            {
                await _bridge.SendErrorAsync(id, "NOT_FOUND", $"Execution not found: {payload.executionId}", token, "uiflow.cancel");
                return;
            }
            ...
            {
                Logger.Log("UIFlow", $"已释放活动执行槽: executionId={payload.executionId}");
            }

            await _bridge.SendResultAsync(id, "uiflow.cancel", CloneExecution(execution), token);
        }

        private async Task ExecuteRunAsync(string executionId, List<string> yamlPaths, UIFlowRunPayload payload, CancellationToken cancellationToken)
        {
            ...
            UIFlow.TestRunnerWindow.ExternalRunStarted(executionId, allPathsForWindow, overallTotal, batchIdx, batchTotal, batchSize);

            try
            {
                var runner = new UIFlow.TestRunner();
                ...
                foreach (string yamlPath in yamlPaths)
                {
                    ...
                    UIFlow.TestRunnerWindow.ExternalCaseStarted(executionId, yamlPath);

                    var testOptions = BuildTestOptions(payload, executionId);
                    testOptions.GenerateSingleReport = yamlPaths.Count == 1;
                    int caseIndex = 1;
                    if (_executions.TryGetValue(executionId, out UIFlowExecutionResultPayload executionBeforeRun))
                    {
                        ...
                        caseResult = await runner.RunFileAsync(
                            ...
                            context =>
                            {
                                lock (_stateLock)
                                {
                                    ...
                                    if (_executions.TryGetValue(executionId, out UIFlowExecutionResultPayload current))
                                    {
                    ...
                    UIFlowCaseResultPayload casePayload = null;
                    if (_executions.TryGetValue(executionId, out UIFlowExecutionResultPayload executionForCase))
                    {
                    ...
                    UIFlow.TestRunnerWindow.ExternalCaseFinished(
                        executionId,
                        yamlPath,
                        caseResult.CaseName,
                        caseResult.Status,
                    ...
                    if (_executions.TryGetValue(executionId, out UIFlowExecutionResultPayload executionAfterCase))
                    {
                    ...
                    try
                    {
                        await _bridge.SendEventAsync(
                            ...
                            "uiflow.case_finished",
                            casePayload,
                            cancellationToken);
                    }
                    catch (Exception evtEx)
                    {
                        Logger.LogWarning("UIFlow", $"发送 case_finished 事件失败: {evtEx.Message}");
                    }
                ...
                {
                    UIFlow.TestRunnerWindow.ExternalRunFinished(
                ...
                {
                    Logger.LogWarning("UIFlow", $"发送 run_completed 事件失败: {evtEx.Message}");
                }
            ...
            catch (OperationCanceledException)
            {
                ...
                if (_executions.TryGetValue(executionId, out var abortedExec))
                    UIFlow.TestRunnerWindow.ExternalRunFinished(executionId, "aborted",
            ...
            catch (Exception ex)
            {
                ...
                if (_executions.TryGetValue(executionId, out var failedExec))
                    UIFlow.TestRunnerWindow.ExternalRunFinished(executionId, "failed",
        ...
        /// </summary>
        private void SyncWindowOnOpen(UIFlow.TestRunnerWindow _)
        {
            string activeId;
            UIFlowExecutionResultPayload snap;
            lock (_stateLock)
            {
            ...
            {
                Enum.TryParse<UIFlow.TestStatus>(c.status, out var st);
                return (
                    yamlPath: c.yamlPath,
                    caseName: c.caseName,
                    status: st,
                    ...
                    stepResults: (List<UIFlow.StepResult>)null,  // not available in snapshot
                    reportMarkdownPath: c.reportMarkdownPath,
                    reportJsonPath: c.reportJsonPath
                );
            });

            UIFlow.TestRunnerWindow.TrySyncActiveRun(
                activeId,
                allYamlPaths,
                totalAll,
        ...
        private void GenerateSuiteReport(string executionId, ReportPathBuilder reportPaths)        {
            if (!_executions.TryGetValue(executionId, out UIFlowExecutionResultPayload execution))
            {
                return;
            }

            var suiteResult = new UIFlow.TestSuiteResult
            {
            ...
            foreach (UIFlowCaseResultPayload casePayload in execution.cases)
            {
                if (Enum.TryParse<UIFlow.TestStatus>(casePayload.status, out UIFlow.TestStatus status))
                {
                    suiteResult.CaseResults.Add(new UIFlow.TestResult
                    {
            ...
            var reporter = new UIFlow.MarkdownReporter(new UIFlow.ReporterOptions
            {
            ...
            try
            {
                ...
                Logger.Log("UIFlow", $"生成套件报告 full_reports.md 路径={execution.reportPath}");
            }
            catch (Exception ex)
            {
                Logger.LogWarning("UIFlow", $"生成套件报告失败: {ex.Message}");
            }

            // Overwrite unified suite report with batch results
            try
            {
                UIFlow.MarkdownReporter.WriteUnifiedSuiteReport(suiteResult, overwrite: true);
                Logger.Log("UIFlow", "生成统一套件报告 Reports/full_reports.md");
            }
            catch (Exception unifiedEx)
            {
                Logger.LogWarning("UIFlow", $"生成统一套件报告失败: {unifiedEx.Message}");
            }
        }

        private static TestOptions BuildTestOptions(UIFlowRunPayload payload, string executionId)
        {
            string reportPath = BuildExecutionReportPath(payload.reportPath, executionId);
        ...
        public static List<string> ResolveYamlPaths(UIFlowRunPayload payload)
        {
        ...
        public static UIFlowCaseResultPayload ToCasePayload(TestResult result, string yamlPath, string reportPath, ReportPathBuilder reportPaths)
        {
            var payload = new UIFlowCaseResultPayload
            {
            ...
            if (result.StepResults != null)
            {
                for (int index = 0; index < result.StepResults.Count; index++)
                {
                    ...
                    var stepPayload = new UIFlowStepResultPayload
                    {
                        stepIndex = index + 1,
        ...
        public static void ApplyCaseCounters(UIFlowExecutionResultPayload execution, TestStatus status)
        {
            switch (status)
            {
        ...
        private void UpdateExecution(string executionId, Action<UIFlowExecutionResultPayload> updater)
        {
            if (!_executions.TryGetValue(executionId, out UIFlowExecutionResultPayload execution))
            {
                return;
        ...
        private UIFlowExecutionResultPayload CloneExecution(UIFlowExecutionResultPayload source)
        {
            lock (_stateLock)
            {
                var clone = new UIFlowExecutionResultPayload
                {
                ...
                foreach (UIFlowCaseResultPayload caseResult in source.cases)
                {
        ...
        private static UIFlowCaseResultPayload CloneCase(UIFlowCaseResultPayload source)
        {
            var clone = new UIFlowCaseResultPayload
            {
            ...
            foreach (UIFlowStepResultPayload step in source.stepResults)
            {
        ...
        private static UIFlowStepResultPayload CloneStep(UIFlowStepResultPayload source)
        {
            var clone = new UIFlowStepResultPayload
            {
