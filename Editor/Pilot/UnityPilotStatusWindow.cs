...
{
    public class UnityPilotStatusWindow : EditorWindow
    {
        ...
        {
            var win = GetWindow<UnityPilotStatusWindow>("upilot");
        ...
        private void OnEnable()
        {
            ...
            UnityPilotMcpServerManager.Instance.ValidateAndAutoFixPath();
        }
        ...
        private static Task<string> CheckPythonMcpProcess()
        {
            try
            {
                ...
                if (unityPilotLikeCount == 0)
                {
                    ...
                    summary.AppendLine("  请确认命令行包含 run_upilot_mcp.py、upilot-mcp、run_unitypilot_mcp.py 或 unitypilot_mcp");
                }
                else
                {
                    summary.AppendLine($"✓ 疑似 upilot MCP 进程数量: {unityPilotLikeCount}");
                }
        ...
        private static bool IsUnityPilotMcpLike(string cmdLine)
        {
            return ContainsIgnoreCase(cmdLine, "run_upilot_mcp.py") ||
                   ...
                   ContainsIgnoreCase(cmdLine, "run_unitypilot_mcp.py") ||
                   ContainsIgnoreCase(cmdLine, "unitypilot_mcp") ||
                   ContainsIgnoreCase(cmdLine, "unitypilot-mcp");
        }
