# UPilot 历史整合快照（归档，不参与当前状态统计）

> 历史快照说明（2026-09-07）：本文件仅保留 2026-08-12 起的整合过程和验收证据，不再作为活动 TODO 清单。
> 当前唯一产品状态源为仓库根 `D:\upilot\TODO_UPilot.mcd`；本文件中的状态、优先级和出现次数不参与当前统计，也不与项目侧现场清单重复计数。
> 2026-09-07 UP-001～UP-009 实施：UP-001 已通过删除与直接回归、出现次数清零；其余八项保持部分完成。当前接口与验收边界见 `UP001-UP009-Delivery.md`，下文历史“已实现”不覆盖新一轮未验收项。
> 2026-09-08 上一批收口（历史）：Unity 2022.3 授权项目定向验收后，UP-003、UP-005 也已完成并清零；当时前九项为 3 项完成、6 项部分完成。
> 2026-09-08 当前收口：UP-008 最终定向 10/10（含真实 Reload 与连续 SceneGUI 像素）通过，出现次数清零；用户明确忽略 UP-002、004、006、007、009 剩余缺口。前九项为 4 完成、5 已忽略；忽略不等于修复，保留原次数和风险，发布/像素安全门禁不变。根清单共 78 项：56 完成、5 部分完成、12 待处理、5 忽略。下文历史证据不改作当前验收结论。
