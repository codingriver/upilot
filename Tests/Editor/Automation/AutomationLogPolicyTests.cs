...
{
    public class AutomationLogPolicyTests
    {
        [Test]
        public void DefaultPolicyBlocksErrorExceptionAndAssert()
        {
            AutomationLogPolicyResult result = AutomationLogPolicy.Evaluate(new[]
            {
        ...
        {
            AutomationLogRule scoped = Rule("case-allow", "allow", -1, "expected");
        ...
        {
            AutomationLogRule scoped = Rule("scoped", "allow", -1, "expected");
            scoped.phaseIds = new[] { "cleanup" };
            scoped.caseIds = new[] { "case-a" };
            AutomationLogPolicyResult result = AutomationLogPolicy.Evaluate(
                new[] { Record(5, "Error", "expected") },
                new[]
                {
                    new AutomationLogInterval { fromSequenceInclusive = 0, toSequenceExclusive = 10, phaseId = "cleanup" },
                    new AutomationLogInterval { fromSequenceInclusive = 2, toSequenceExclusive = 8, caseId = "case-a" },
                },
                new[] { scoped });
        ...
        public void FingerprintsNormalizeGuidWhitespaceAndStackLinesButKeepBusinessIds()
        {
            ...
            Assert.That(AutomationLogPolicy.Fingerprint(first), Is.EqualTo(AutomationLogPolicy.Fingerprint(equivalent)));
            Assert.That(AutomationLogPolicy.Fingerprint(first), Is.Not.EqualTo(AutomationLogPolicy.Fingerprint(otherBusinessId)));
        }
        ...
        private static AutomationLogInterval Interval(long from, long to, string caseId) => new()
        {
        ...
        private static AutomationLogRule Rule(string id, string action, int maximumCount, string contains) => new()
        {
            id = id,
            action = action,
            levels = new[] { "Error" },
