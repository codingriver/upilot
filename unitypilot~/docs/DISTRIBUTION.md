...
```text
<浠撳簱鏍?/
  upilot/      # Unity UPM 鍖咃紙package.json + Editor/ C# 鎻掍欢锛?  src/unitypilot_mcp/     # Python 婧愮爜
    __init__.py
  ...
  UnityPilot/             # 鍙€夛細绀轰緥 Unity 宸ョ▼锛堟湰鍦?file: 寮曠敤 upilot 锛?```
...
鏈粨搴撳嵆 UPM 鍖呮潵婧愶細鍖呮牴鐩綍涓?`upilot/`锛堝唴鍚?`package.json`锛夛紝涓?GitHub 杩滅▼涓€鑷淬€?
...
```json
"io.github.codingriver.upilot": "file:../../upilot"
```
