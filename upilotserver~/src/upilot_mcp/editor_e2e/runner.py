...
logger = logging.getLogger("upilot.editor_e2e.runner")
...
async def run_editor_e2e_from_path(
    ...
    art = Path(artifact_dir).expanduser().resolve() if artifact_dir else (spec_dir / ".upilot-e2e-artifacts" / run_id)
    ...
    if not passed and webhook_on_failure:
        wh = getenv("UPILOT_E2E_WEBHOOK_URL", "").strip()
        if wh:
            try:
...
def _post_e2e_webhook(url: str, report_path: Path, artifact_dir: Path) -> None:
    ...
    boundary = f"----upilot-{uuid.uuid4().hex}"
    crlf = "\r\n"
    parts: list[str] = []
